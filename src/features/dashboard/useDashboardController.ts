...
export function useDashboardController(result: any) {
  ...
  const [celebrityResults, setCelebrityResults] = useState<any[]>(
    result.visionAnalysis?.celebritySimilarity || []
  );
  ...
  const potentialScore =
    result.visionAnalysis?.potentialScore || Math.min(10, Math.max(9.2, overallScore + 1.2));
