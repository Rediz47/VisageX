...
export default function BlogAnalysisPage() {
  ...
  const Card = ({ icon: Icon, title, children }: { icon: any, title: string, children: React.ReactNode }) => (
    <div className={`p-8 md:p-10 rounded-[2.5rem] border transition-all duration-500 hover:-translate-y-1 ${isDarkMode ? 'bg-zinc-900/50 border-white/5 hover:bg-white/[0.07]' : 'bg-white border-zinc-200 shadow-xl hover:shadow-2xl'}`}>
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-8 ${isDarkMode ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card icon={ScanFace} title="Step 1: The 468-Point Mesh">
              <p>When you upload a photo to an <Link to="/" className="text-indigo-500 font-bold hover:underline">AI face analysis tool</Link>, the neural network bypasses basic detection. Standard tech just finds your eyes. Modern AI drops exactly <strong>468 geometric anchors</strong> onto your features.</p>
              <p>These points track the exact curve of your jawline, the width of your nose base, and the resting tilt of your eyes. This dense mesh forms the foundation of every subsequent aesthetic calculation.</p>
            </Card>

            <Card icon={Activity} title="Step 2: Golden Ratio Geometry">
              <p>With the geometry locked in, the AI calculates the spatial relationships between your features:</p>
              <ul className="list-disc pl-5 mt-2 space-y-2">
                <li><strong>Symmetry:</strong> Measures the delta (in millimeters) between the left and right sides of your jaw and cheekbones.</li>
                <li><strong>Facial Thirds:</strong> Checks if hairline-to-brow, brow-to-nose, and nose-to-chin are perfectly balanced.</li>
                <li><strong>Canthal Tilt:</strong> Calculates the angle of your eyes to determine dominance.</li>
              </ul>
            </Card>

            <Card icon={Camera} title="Step 3: Dermatological Pixels">
              <p>Beyond bone structure geometry, specialized vision models analyze the actual texture of your skin's pixels. The AI scans for contrast irregularities that indicate <strong>dark circles, hyper-pigmentation, and acne presence</strong>.</p>
              <p>It generates a density map measuring oiliness and redness to output a final, objective "Skin Quality Score".</p>
            </Card>

            <Card icon={Settings} title="Step 4: The Glow-Up Engine">
              <p>The real magic of looksmaxxing AI is the synthesis. By processing your structural weaknesses and skin flaws, the AI creates a customized aesthetic routine.</p>
              <p>For example, if it detects a high forehead and a triangle face shape, it automatically triggers specific hairstyle recommendations that add volume exactly where your face needs balancing.</p>
            </Card>
          </div>
