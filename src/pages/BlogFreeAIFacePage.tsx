...
export default function BlogFreeAIFacePage() {
  ...
  const Card = ({
    ...
    >
      ...
      <div
        className={`space-y-3 text-base leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}
      >
        {children}
      </div>
    </div>
    ...
    >
      ...
      <article className="pt-32 pb-20 px-6 max-w-5xl mx-auto">
        ...
        >
          ...
          {/* Hero callout */}
          <div
            className={`p-8 md:p-12 rounded-[3rem] border text-center ${isDarkMode ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-indigo-50 border-indigo-100'}`}
          >
            <p
          ...
          <div className="space-y-8">
            <div className="text-center">
              <h2
                className={`text-3xl md:text-4xl font-display font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}
              >
                What Real AI Face Analysis Actually Measures
              </h2>
              <p
                className={`text-lg font-light max-w-2xl mx-auto ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}
              >
                The difference between a real tool and a fake one comes down to what's being
                computed.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card icon={Brain} title="Facial Landmark Detection" color="indigo">
                <p>
                  A proper tool uses a MediaPipe or similar neural network to map 468 distinct
                  points across your face — eyes, nose, jaw, cheekbones, forehead. This mesh becomes
                  the foundation for every score.
                </p>
                <p className="font-medium">
                  Tools with fewer than 100 landmarks are not doing real analysis.
                </p>
              </Card>
              <Card icon={Sparkles} title="Geometric Ratio Scoring" color="violet">
                <p>
                  Using the landmark mesh, the AI calculates: facial thirds (forehead:nose:chin
                  ratios), the golden ratio (1.618:1 facial proportions), canthal tilt, facial
                  width-to-height ratio (fWHR), and jaw angle.
                </p>
              </Card>
              ...
              <Card icon={Zap} title="Potential & Personalization" color="amber">
                <p>
                  Beyond a raw score, legitimate tools identify your face shape, color season,
                  specific visual strengths and weaknesses, and generate a personalized improvement
                  roadmap. Generic tools give you none of this.
                </p>
              </Card>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <h2
              className={`text-3xl font-display font-bold text-center ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}
            >
              Free AI Face Analysis Tools Compared (2026)
            </h2>
            <div
              className={`rounded-[2.5rem] border overflow-hidden ${isDarkMode ? 'border-white/10' : 'border-zinc-200 shadow-xl'}`}
            >
              <table className="w-full text-sm">
                <thead>
                  <tr
                    className={`text-left ${isDarkMode ? 'bg-white/5 text-zinc-400' : 'bg-zinc-50 text-zinc-500'}`}
                  >
                    ...
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-xs">
                      Landmarks
                    </th>
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-xs">
                      Skin AI
                ...
                <tbody>
                  {tools.map((t, i) => (
                    <tr
                      key={i}
                      className={`border-t ${isDarkMode ? 'border-white/5' : 'border-zinc-100'} ${i === 0 ? (isDarkMode ? 'bg-indigo-500/5' : 'bg-indigo-50/50') : ''}`}
                    >
                      <td
                        className={`px-6 py-5 font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}
                      >
                        {t.name}
                        {i === 0 && (
                          <span className="ml-2 text-[10px] font-bold uppercase tracking-widest text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded-full">
                            This article
                          </span>
                        )}
                      </td>
                      ...
                      <td
                        className={`px-6 py-5 font-mono text-xs ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}
                      >
                        {t.landmarks}
                      </td>
                      <td className="px-6 py-5">
                      ...
                      <td
                        className={`px-6 py-5 text-xs font-bold ${i === 0 ? 'text-indigo-400' : isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}
                      >
                        {t.verdict}
                      </td>
                    </tr>
          ...
          >
            ...
            <div className="space-y-8">
              {[
                ...
                <div
                  key={i}
                  className={`pb-8 border-b last:border-0 last:pb-0 ${isDarkMode ? 'border-white/5' : 'border-zinc-100'}`}
                >
                  <h3
                    className={`text-lg font-bold mb-3 ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}
                  >
                    {faq.q}
                  </h3>
                  <p
          ...
          >
            <div
              className={`absolute inset-0 opacity-50 transition-opacity duration-700 group-hover:opacity-100 ${isDarkMode ? 'bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-indigo-500/20 via-transparent to-transparent' : ''}`}
            />
