...
export default function BlogRecessedJawPage() {
  ...
  const Card = ({ icon: Icon, title, children }: { icon: any, title: string, children: React.ReactNode }) => (
    <div className={`p-8 md:p-10 rounded-[2.5rem] border transition-all duration-500 hover:-translate-y-1 ${isDarkMode ? 'bg-zinc-900/50 border-white/5 hover:bg-white/[0.07]' : 'bg-white border-zinc-200 shadow-xl hover:shadow-2xl'}`}>
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-8 ${isDarkMode ? 'bg-rose-500/20 text-rose-400' : 'bg-rose-50 text-rose-600'}`}>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card icon={ActivitySquare} title="Mouth Breathing (During Childhood)">
                <p>The number one cause of a recessed jawline is mouth breathing during developmental years. When the mouth is open, the tongue drops and fails to support the maxilla. The facial bones grow long and downwards rather than horizontally forward.</p>
              </Card>
              <Card icon={ShieldCheck} title="Genetic Underbite / Overbite">
                <p>Structural malocclusions (how your teeth fit together) often dictate the positioning of the mandible. A severe overbite pushes the lower jaw backward in relation to the rest of the face, leading to a weak chin profile.</p>
              </Card>
            </div>
          </div>

          <div className={`p-10 md:p-14 rounded-[3rem] border ${isDarkMode ? 'bg-white/[0.02] border-white/10' : 'bg-white border-zinc-200'}`}>
            <h2 className={`text-3xl md:text-4xl font-display mb-10 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>Three Non-Surgical Solutions</h2>
            <div className="space-y-12">
              <div>
                <h3 className={`text-xl font-bold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-rose-400' : 'text-rose-600'}`}><Dumbbell className="w-5 h-5"/> Masseter Hypertrophy</h3>
                <p className={`text-base leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  If your jawline lacks *width* rather than forward projection, you can build the masseter muscles through tough chewing (Mastic gum). However, if your jaw is severely recessed backwards, building the muscle without fixing projection can make the face look abnormally blocky.
                </p>
              </div>
              <div>
                <h3 className={`text-xl font-bold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-rose-400' : 'text-rose-600'}`}><ShieldCheck className="w-5 h-5"/> Correct Orthodontics</h3>
                <p className={`text-base leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  For adults, clear aligners or braces can fix the bite, which subtly changes how your lower jaw pulls against the joint. While it won't give you model-tier bone structure on its own, fixing an overbite provides a noticeable improvement to jawline projection.
                </p>
              </div>
              <div>
                <h3 className={`text-xl font-bold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-rose-400' : 'text-rose-600'}`}><MoveDown className="w-5 h-5"/> Posture Tucking (Mewing)</h3>
                <p className={`text-base leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  Correcting your neck posture immediately stretches the skin under the chin, reducing the appearance of a double chin. Mewing (pressing the tongue to the roof of the mouth) lifts the submental tissue rapidly, making the jawline appear much sharper in side profile immediately, even before structural bone changes occur.
                </p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative z-10 max-w-2xl mx-auto space-y-8">
              ...
              <p className={`text-lg md:text-xl font-light ${isDarkMode ? 'text-zinc-300' : 'text-zinc-600'}`}>
                Stop holding your phone above your head to hide your jawline. Upload a raw selfie to our engine and instantly get your exact jawline angularity matrix.
              </p>
