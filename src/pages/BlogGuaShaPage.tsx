...
export default function BlogGuaShaPage() {
  ...
  const Card = ({ icon: Icon, title, children }: { icon: any, title: string, children: React.ReactNode }) => (
    <div className={`p-8 md:p-10 rounded-[2.5rem] border transition-all duration-500 hover:-translate-y-1 ${isDarkMode ? 'bg-zinc-900/50 border-white/5 hover:bg-white/[0.07]' : 'bg-white border-zinc-200 shadow-xl hover:shadow-2xl'}`}>
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-8 ${isDarkMode ? 'bg-purple-500/20 text-purple-400' : 'bg-purple-50 text-purple-600'}`}>
      ...
      <article className="pt-32 pb-20 px-6 max-w-5xl mx-auto">
        ...
        >
          ...
          <header className="text-center max-w-3xl mx-auto space-y-6">
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest border ${isDarkMode ? 'border-purple-500/30 text-purple-400 bg-purple-500/10' : 'border-purple-200 text-purple-600 bg-purple-50'}`}>
            ...
            <p className={`text-lg md:text-xl font-light max-w-2xl mx-auto ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
              TikTok influencers claim it redesigns your jawline. The reality is far less magical, but still incredibly useful for your facial harmony.
            </p>
          </header>
          ...
          <div className={`w-full aspect-[21/9] rounded-[3rem] overflow-hidden flex flex-col items-center justify-center border-2 border-dashed ${isDarkMode ? 'border-white/10 bg-white/[0.02]' : 'border-zinc-300 bg-zinc-100'} transition-all hover:border-zinc-500`}>
            {/* If you have a photo, delete this entire <div> block and replace with: 
                <img src="/my-gua-sha-cover.jpg" className="w-full h-full object-cover" /> 
            */}
            <ImagePlus className={`w-12 h-12 mb-4 ${isDarkMode ? 'text-zinc-600' : 'text-zinc-400'}`} />
            <p className={`text-lg font-bold tracking-widest uppercase ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>[ Insert Tutorial Photo ]</p>
            <p className={`text-sm mt-2 font-mono ${isDarkMode ? 'text-zinc-600' : 'text-zinc-400'}`}>&lt;img src="/gua-sha-tutorial.jpg" /&gt;</p>
          </div>

          <div className="space-y-8">
            <h2 className={`text-3xl font-display text-center ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>The Medical Truth</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card icon={Scale} title="It Does Not Burn Adipose Tissue">
                <p>Let's clear the air immediately. <strong>Scraping a stone crystal against your skin cannot spot-reduce adipose tissue (fat cells).</strong> If you have high body fat percentage, Gua Sha will not melt away the fat underneath your chin or cheeks. The only way to lose genetic face fat is to enter a caloric deficit through diet and exercise.</p>
              </Card>
              <Card icon={Droplets} title="It Excels at Lymphatic Drainage">
                <p>So why do people go from puffy to sharp in a 3-minute video? Liquid retention. The lymphatic system has no "pump" like the heart. <strong>Gua Sha physically pushes stagnant fluid buildup</strong> out from under the skin down into the lymph nodes in the neck, instantly "de-puffing" the face and revealing the bone structure that was hiding underneath.</p>
              </Card>
            </div>
          </div>

          <div className={`p-10 md:p-14 rounded-[3rem] border ${isDarkMode ? 'bg-white/[0.02] border-white/10' : 'bg-white border-zinc-200'}`}>
            <h2 className={`text-3xl md:text-4xl font-display mb-10 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>How to Actually Define Your Jawline</h2>
            <div className="space-y-12">
              <div>
                <h3 className={`text-xl font-bold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`}><Hand className="w-5 h-5"/> Step 1: Drain the Inflammation</h3>
                <p className={`text-base leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  Use a Gua Sha in the morning. Apply a facial oil so the stone glides. Anchor your skin with one hand and sweep the stone outwards from your chin to your earlobe. Always drain <strong>down the neck</strong>. This clears morning puffiness.
                </p>
              </div>
              <div>
                <h3 className={`text-xl font-bold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`}><Droplets className="w-5 h-5"/> Step 2: Limit Sodium Intake</h3>
                <p className={`text-base leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                  Eating high amounts of sodium and processed carbs late at night binds to water. This pulls water directly into the extracellular space of your face, completely hiding your cheekbones. The "moon face" look is frequently a dietary hydration issue, not a bone structure defect.
                </p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative z-10 max-w-2xl mx-auto space-y-8">
              ...
              <p className={`text-lg md:text-xl font-light ${isDarkMode ? 'text-zinc-300' : 'text-zinc-600'}`}>
                Our system calculates skin density, shadows, and precise jawline geometry so you can figure out what is really going on below the skin.
              </p>
