...
interface Ring { x: number; y: number; r: number; maxR: number; a: number; col: string; w: number; alive: boolean }
interface Dot { id: number; x: number; y: number; born: number; col: string; alive: boolean }
interface Spark { x: number; y: number; vx: number; vy: number; life: number; col: string; kind: 'soft' | 'data' | 'burst'; alive: boolean }
interface Wireframe {
  name: string;
  pts: Float32Array;
  count: number;
...
function ik2(sx: number, sy: number, tx: number, ty: number, u: number, l: number, sign: number): Vec2 {
...
const FB: Record<number, Vec2> = {
  10: { x: 0.50, y: 0.18 }, 9: { x: 0.50, y: 0.33 },
  70: { x: 0.40, y: 0.32 }, 300: { x: 0.60, y: 0.32 },
  33: { x: 0.38, y: 0.40 }, 263: { x: 0.62, y: 0.40 },
  168: { x: 0.50, y: 0.42 }, 1: { x: 0.50, y: 0.55 }, 2: { x: 0.50, y: 0.60 },
  234: { x: 0.30, y: 0.57 }, 454: { x: 0.70, y: 0.57 },
  61: { x: 0.42, y: 0.68 }, 291: { x: 0.58, y: 0.68 },
  152: { x: 0.50, y: 0.82 },
};
const FB_EXTRA: Record<number, Vec2> = {
  107: { x: 0.44, y: 0.29 }, 55: { x: 0.47, y: 0.31 }, 285: { x: 0.53, y: 0.31 },
  6: { x: 0.50, y: 0.37 }, 197: { x: 0.50, y: 0.42 }, 195: { x: 0.50, y: 0.47 },
  5: { x: 0.50, y: 0.50 }, 4: { x: 0.50, y: 0.52 },
  132: { x: 0.26, y: 0.63 }, 172: { x: 0.28, y: 0.70 }, 136: { x: 0.32, y: 0.75 },
  150: { x: 0.38, y: 0.79 }, 149: { x: 0.44, y: 0.81 }, 176: { x: 0.48, y: 0.82 },
  148: { x: 0.50, y: 0.82 }, 0: { x: 0.50, y: 0.70 },
};
...
const WIREFRAME_GROUPS: WireframeGroup[] = [
  ...
  { name: 'leftEye',   ids: [33, 246, 161, 160, 159, 158, 157, 173, 133, 155, 154, 153, 145, 144, 163, 7],
    col: '#22d3ee', closed: true, coreWidth: 1.3, glowWidth: 3.2 },
  // Right eye — closed MediaPipe right-eye ring
  { name: 'rightEye',  ids: [263, 466, 388, 387, 386, 385, 384, 398, 362, 382, 381, 380, 374, 373, 390, 249],
    col: '#38bdf8', closed: true, coreWidth: 1.3, glowWidth: 3.2 },
  // Left brow
  { name: 'leftBrow',  ids: [70, 63, 105, 66, 107], col: '#818cf8', closed: false, coreWidth: 1.2, glowWidth: 3.0 },
  // Right brow
  { name: 'rightBrow', ids: [336, 296, 334, 293, 300], col: '#a78bfa', closed: false, coreWidth: 1.2, glowWidth: 3.0 },
  // Nose bridge
  { name: 'noseBridge', ids: [168, 6, 197, 195, 5, 4], col: '#2dd4bf', closed: false, coreWidth: 1.2, glowWidth: 3.0 },
  // Outer lips — closed loop
  { name: 'outerLips', ids: [61, 185, 40, 39, 37, 0, 267, 269, 270, 409, 291, 375, 321, 405, 314, 17, 84, 181, 91, 146],
    col: '#f472b6', closed: true, coreWidth: 1.3, glowWidth: 3.4 },
];
...
export function RobotScanCanvas({ imageUrl, progress, isDarkMode, landmarks, width = 430, height = 573 }: Props) {
  ...
  useEffect(() => { darkRef.current = isDarkMode; }, [isDarkMode]);
  ...
  const S = useRef({
    ...
    facing: 1 as 1 | -1,
    scale: 0.7, vScale: 0, tScale: 0.7,
    gait: 0, speed: 0,
    levitate: 0, vLevitate: 0, tLevitate: 0,
    headRot: 0, vHeadRot: 0, tHeadRot: 0,
    eyeX: 0, vEyeX: 0, eyeY: 0, vEyeY: 0,
    armExtend: 0, vArmExtend: 0, tArmExtend: 0,
    handX: 0, handY: 0, crossHandX: 0, crossHandY: 0, crossTargetX: 0, crossTargetY: 0,
    phase: 'idle' as Phase, phaseT: 0,
    subPhase: null as SubPhase, subT: 0, subDuration: 0,
    visitIdx: 0,
    focusX: 0, focusY: 0, focusCol: '#22d3ee',
    attention: 0, vAttention: 0, energy: 0,
    primaryBeamOn: false, crossBeamOn: false, readoutOn: false,
    blink: 0, blinkTimer: 0,
    antX: 0, antY: 0, antVx: 0, antVy: 0,
    antX2: 0, antY2: 0, antVx2: 0, antVy2: 0,
    paraX: 0, vParaX: 0,
    rings: [] as Ring[],
    dots: [] as Dot[],
    sparks: [] as Spark[],
    ...
    stallTimer: 0,
    hudWave: new Float32Array(32), hudWaveHead: 0,
    displayText: 'BOOT',
    browTilt: 0, vBrowTilt: 0, tBrowTilt: 0,
    eyeWide: 0, vEyeWide: 0, tEyeWide: 0,
    mouthCurve: 0, vMouthCurve: 0, tMouthCurve: 0,
    fidget: null as Fidget, fidgetT: 0, fidgetNext: 1800, fidgetValue: 0,
    nodImpulse: 0, recoilImpulse: 0, startleImpulse: 0, shakeImpulse: 0,
    lastVisitIdx: -1,
    lastSummaryBeat: -1,
    // Finale
    finaleT: 0,
  ...
  useEffect(() => {
    ...
    const PM = 12;
    const photoX = PM, photoY = PM, photoW = width - PM * 2, photoH = height - PM * 2;

    const GRID_STEP = 26;
    s.grid = { step: GRID_STEP, cols: Math.ceil(photoW / GRID_STEP) + 1, rows: Math.ceil(photoH / GRID_STEP) + 1, ox: photoX, oy: photoY };

    if (!s.init) {
      s.x = photoX + photoW * 0.5;
      s.y = photoY + photoH * 0.78;
      s.tx = s.x; s.ty = s.y;
      s.antX = s.x; s.antY = s.y - 52;
      s.antX2 = s.x; s.antY2 = s.y - 48;
      s.init = true;
    }
    ...
    const pushRing = (x: number, y: number, maxR: number, col: string, w: number) => {
      ...
      for (const r of s.rings) if (!r.alive) { slot = r; break; }
      if (!slot && s.rings.length < RING_CAP) { s.rings.push({ x, y, r: 0, maxR, a: 1, col, w, alive: true }); return; }
      if (!slot) return;
      slot.x = x; slot.y = y; slot.r = 0; slot.maxR = maxR; slot.a = 1; slot.col = col; slot.w = w; slot.alive = true;
    };
    const pushDot = (id: number, x: number, y: number, col: string, now: number) => {
      for (const d of s.dots) if (d.alive && d.id === id) { d.x = x; d.y = y; d.born = now; d.col = col; return; }
      let slot: Dot | null = null;
      for (const d of s.dots) if (!d.alive) { slot = d; break; }
      if (!slot && s.dots.length < DOT_CAP) { s.dots.push({ id, x, y, born: now, col, alive: true }); return; }
      if (!slot) return;
      slot.id = id; slot.x = x; slot.y = y; slot.born = now; slot.col = col; slot.alive = true;
    };
    const pushSpark = (x: number, y: number, col: string, kind: 'soft' | 'data' | 'burst' = 'soft') => {
      let slot: Spark | null = null;
      for (const sp of s.sparks) if (!sp.alive) { slot = sp; break; }
      let vx: number, vy: number;
      if (kind === 'data') { vy = rng(-2.0, -1.0); vx = rng(-0.25, 0.25); }
      else if (kind === 'burst') {
        const ang = rng(0, Math.PI * 2); const sp = rng(2.0, 4.5);
        vx = Math.cos(ang) * sp; vy = Math.sin(ang) * sp;
      } else { vy = rng(-1.2, 0.2); vx = rng(-0.8, 0.8); }
      if (!slot && s.sparks.length < SPARK_CAP) { s.sparks.push({ x, y, vx, vy, life: 1, col, kind, alive: true }); return; }
      if (!slot) return;
      slot.x = x; slot.y = y; slot.vx = vx; slot.vy = vy; slot.life = 1; slot.col = col; slot.kind = kind; slot.alive = true;
    };
    ...
    const buildSmoothedPath = (ids: number[], closed: boolean): { pts: Float32Array; count: number; cumLen: Float32Array; totalLen: number } | null => {
      const n = ids.length;
      if (n < 2) return null;
      ...
      for (let i = 0; i < segCount; i++) {
        const P0 = getA(i - 1), P1 = getA(i), P2 = getA(i + 1), P3 = getA(i + 2);
      ...
      pts[idx * 2] = last.x; pts[idx * 2 + 1] = last.y;
      if (idx > 0) totalLen += Math.hypot(last.x - pts[(idx - 1) * 2], last.y - pts[(idx - 1) * 2 + 1]);
      cumLen[idx] = totalLen;
      idx++;
    ...
    const enterSub = (sub: SubPhase) => {
      ...
      s.displayText = sub === 'calibrate' ? 'CALIB' : sub === 'primaryBeam' ? 'SCAN' : sub === 'crossCheck' ? 'XCHK' : sub === 'readout' ? 'READ' : s.displayText;
    ...
    const nextTarget = () => {
      ...
      const p = pt(id);
      s.focusX = p.x; s.focusY = p.y; s.focusCol = meta.col;
      const side = p.x < photoX + photoW * 0.5 ? 1 : -1;
      const offsetX = side * 46;
      const offsetY = Math.min(60, (photoY + photoH - 30) - p.y) < 20 ? -40 : 40;
      s.tx = clamp(p.x + offsetX, photoX + 30, photoX + photoW - 30);
      s.ty = clamp(p.y + offsetY, photoY + 40, photoY + photoH - 24);
      s.facing = p.x >= s.tx ? 1 : -1;
      const dx = s.tx - s.x, dy = s.ty - s.y;
      const mag = Math.hypot(dx, dy);
      if (mag > 1) { s.vx -= (dx / mag) * 0.6; s.vy -= (dy / mag) * 0.4; }
      if (mag > 200) { s.startleImpulse = 1; s.vy -= 1.5; }
      s.phase = 'moving'; s.phaseT = 0; s.subPhase = null;
    ...
    const tryStartFidget = () => {
      ...
      s.fidgetT = 0; s.fidgetValue = 0;
      s.fidgetNext = 1800 + Math.random() * 2000;
      s.displayText = s.fidget === 'checkWrist' ? 'WRIST' : s.fidget === 'glance' ? 'HI :)' : 'IDLE';
    };

    const updatePace = (now: number, progress: number) => {
      if (s.progHistory.sampledAt === 0) { s.progHistory.p = progress; s.progHistory.t = now; s.progHistory.sampledAt = now; }
      if (now - s.progHistory.sampledAt > 250) {
        const dp = progress - s.progHistory.p;
        const dtp = Math.max(1, now - s.progHistory.t);
        const rate = dp / dtp;
        s.progHistory.p = progress; s.progHistory.t = now; s.progHistory.sampledAt = now;
        if (rate < 1e-5) s.stallTimer += 250; else s.stallTimer = 0;
        ...
        if (s.stallTimer > 3000 || rate < 1e-5) paceTarget = 1.5;
          ...
          const subTotal = SUB_BASE.calibrate + SUB_BASE.primaryBeam + SUB_BASE.crossCheck + SUB_BASE.readout + T_ANALYZE_BASE;
          const workMs = remainingLandmarks * subTotal + T_WIREFRAME_BASE * (1 + (WIREFRAME_GROUPS.length - 1) * WIREFRAME_STAGGER) + T_SUMMARY_BASE;
    ...
    const tick = (now: number) => {
      const dt = clamp(now - (lastT.current || now - 16), 1, 50);
      lastT.current = now;
      s.phaseT += dt; s.subT += dt; s.fidgetT += dt;
      ...
      s.nodImpulse *= 0.9; s.recoilImpulse *= 0.88; s.startleImpulse *= 0.87; s.shakeImpulse *= 0.86;

      const atAllDone = s.visitIdx >= VISIT_ORDER.length;
      if (!atAllDone && s.phase === 'idle') {
        if (s.phaseT > (s.lastVisitIdx < 0 ? 400 : T_IDLE_HOP)) nextTarget();
        ...
        if (landmarksReady) {
          ...
          s.phase = 'wireframe'; s.phaseT = 0; s.wireframeIdx = 0;
          s.displayText = 'MESH';
          s.tx = photoX + photoW * 0.5; s.ty = photoY + photoH * 0.32;
        ...
        if (dist < 2.5 && Math.hypot(s.vx, s.vy) < 0.4) { s.phase = 'scanning'; s.phaseT = 0; enterSub('calibrate'); }
        ...
        if (s.subT > s.subDuration) {
          if (s.subPhase === 'calibrate') enterSub('primaryBeam');
            ...
            s.nodImpulse = 1; s.shakeImpulse = 0.6;
            s.phase = 'analyzing'; s.phaseT = 0;
            s.subPhase = null; s.primaryBeamOn = false; s.crossBeamOn = false; s.readoutOn = false;
            s.displayText = 'OK';
          }
        }
        if (s.subPhase === 'calibrate') s.tx += Math.sign(s.focusX - s.x) * 0.06;
        if (s.subPhase === 'readout' && Math.random() < 0.55) pushSpark(s.focusX + rng(-3, 3), s.focusY + rng(-3, 3), s.focusCol, 'data');
        if (s.subPhase === 'primaryBeam' && Math.random() < 0.04) { pushRing(s.focusX, s.focusY, 26, s.focusCol, 1); s.recoilImpulse = Math.max(s.recoilImpulse, 0.6); }
        const pdx = s.focusX - s.handX, pdy = s.focusY - s.handY;
        ...
        if (s.phaseT > dur) { s.visitIdx += 1; s.phase = 'idle'; s.phaseT = 0; }
        ...
        for (const w of s.wireframes) if (w.alive) {
        ...
        if (leading) {
          ...
          let lo = 0, hi = leading.count - 1;
          while (lo < hi) { const m = (lo + hi) >> 1; if (leading.cumLen[m] < visible) lo = m + 1; else hi = m; }
          s.focusX = leading.pts[lo * 2]; s.focusY = leading.pts[lo * 2 + 1]; s.focusCol = leading.col;
        }
        ...
        if (allDone && s.phaseT > lastStartAt + dur) {
          for (const w of s.wireframes) w.alive = false;
          s.phase = 'summary'; s.phaseT = 0; s.displayText = 'ANLYZ';
          s.primaryBeamOn = false;
          s.tx = photoX + photoW * 0.5; s.ty = photoY + photoH * 0.5;
          s.focusX = photoX + photoW * 0.5; s.focusY = photoY + photoH * 0.5;
        }
        ...
        if (s.phaseT > dur) {
          s.phase = 'finale'; s.phaseT = 0;
          s.finaleT = 0; s.finaleStage = 1;
          s.finaleBurstSpawned = false;
          s.tx = photoX + photoW * 0.5; s.ty = photoY + photoH * 0.55;
          s.displayText = 'COMPL';
        }
        ...
        if (s.finaleT < T_STAGES[0]) {
          ...
          if (!s.finaleBurstSpawned && s.finaleT > 300) {
            ...
            for (let i = 0; i < 48; i++) pushSpark(cx + rng(-1, 1), cy + rng(-1, 1), i % 2 ? '#ffffff' : '#7dd3fc', 'burst');
            s.finaleBurstSpawned = true;
            // Dot flare rings
            for (const d of s.dots) if (d.alive) { pushRing(d.x, d.y, 26, '#ffffff', 1.4); pushRing(d.x, d.y, 46, d.col, 1); }
          }
          s.displayText = 'COMPL';
        }
        ...
        // Stage 4: Backflip (3400 .. 4400)
          ...
          if (Math.random() < 0.35) pushSpark(s.x + rng(-4, 4), s.y - 8 + rng(-4, 4), '#7dd3fc', 'soft');
          s.displayText = ':)';
        }
        // Stage 5: Settle (4400 .. 5000)
        else if (s.finaleT < T_STAGES[4]) {
          s.finaleStage = 5;
          s.finaleSpin = 0; s.finaleLift = 0;
          s.tx = photoX + photoW * 0.5; s.ty = photoY + photoH * 0.78;
          ...
          s.finaleStage = 0;
          s.finaleSpin = 0; s.finaleLift = 0; s.flashAlpha = 0; s.showHoloDone = 0;
          s.displayText = 'ONLINE';
        }
      }

      // Motion springs
      const hovering = s.phase === 'scanning' || s.phase === 'analyzing' || s.phase === 'wireframe' || s.phase === 'summary' || (s.phase === 'finale' && s.finaleStage >= 1 && s.finaleStage <= 3);
      const levitateY = hovering ? -8 : 0;
      const targetXraw = s.tx;
      const targetYraw = s.ty + levitateY + (s.levitate > 0.3 ? Math.sin(now * 0.004) * 1.4 : 0);
      if (s.recoilImpulse > 0.05) {
        const dx = s.x - s.focusX, dy = s.y - s.focusY;
        const l = Math.hypot(dx, dy) || 1;
        s.vx += (dx / l) * 0.25 * s.recoilImpulse;
        s.vy += (dy / l) * 0.15 * s.recoilImpulse;
      }
      { const r = spring(s.x, s.vx, targetXraw, 0.05, 0.82); s.x = r.v; s.vx = r.vel; }
      { const r = spring(s.y, s.vy, targetYraw, 0.05, 0.82); s.y = r.v; s.vy = r.vel; }
      s.speed = clamp(Math.hypot(s.vx, s.vy) * 0.6, 0, 1);

      s.tScale = s.phase === 'scanning' ? 0.68 : 0.7;
      { const r = spring(s.scale, s.vScale, s.tScale, 0.08, 0.78); s.scale = r.v; s.vScale = r.vel; }

      s.tLevitate = hovering ? 1 : 0;
      { const r = spring(s.levitate, s.vLevitate, s.tLevitate, 0.07, 0.8); s.levitate = r.v; s.vLevitate = r.vel; }

      const hy = s.y - 34;
      const hx = s.x + s.facing * 2;
      s.tHeadRot = Math.atan2(s.focusY - hy, s.focusX - hx);
      { const r = spring(s.headRot, s.vHeadRot, s.tHeadRot, 0.12, 0.72); s.headRot = r.v; s.vHeadRot = r.vel; }
      { const r = spring(s.eyeX, s.vEyeX, Math.cos(s.headRot) * 1.6, 0.08, 0.78); s.eyeX = r.v; s.vEyeX = r.vel; }
      { const r = spring(s.eyeY, s.vEyeY, Math.sin(s.headRot) * 1.2, 0.08, 0.78); s.eyeY = r.v; s.vEyeY = r.vel; }

      s.tArmExtend = s.primaryBeamOn ? 1 : (s.phase === 'wireframe' ? 1 : 0);
      { const r = spring(s.armExtend, s.vArmExtend, s.tArmExtend, 0.12, 0.75); s.armExtend = r.v; s.vArmExtend = r.vel; }

      const tAtt = (s.subPhase === 'primaryBeam' || s.subPhase === 'crossCheck' || s.phase === 'wireframe') ? 1 : (s.subPhase === 'readout' ? 0.6 : 0);
      { const r = spring(s.attention, s.vAttention, tAtt, 0.08, 0.8); s.attention = r.v; s.vAttention = r.vel; }

      const tPara = (s.focusX - width * 0.5) * 0.02;
      { const r = spring(s.paraX, s.vParaX, tPara, 0.04, 0.88); s.paraX = r.v; s.vParaX = r.vel; }

      s.tBrowTilt = s.phase === 'scanning' ? -0.22 : s.phase === 'moving' ? 0.14 : s.celebrate ? 0.25 : (s.fidget === 'glance' ? 0.10 : 0);
      s.tEyeWide = s.phase === 'scanning' ? -0.30 : s.phase === 'moving' ? 0.25 : s.startleImpulse > 0.3 ? 0.80 : s.nodImpulse > 0.5 ? 0.60 : s.celebrate ? -0.15 : 0;
      s.tMouthCurve = s.celebrate ? 0.45 : s.phase === 'scanning' ? -0.10 : s.fidget === 'glance' ? 0.30 : 0;
      { const r = spring(s.browTilt, s.vBrowTilt, s.tBrowTilt, 0.12, 0.75); s.browTilt = r.v; s.vBrowTilt = r.vel; }
      { const r = spring(s.eyeWide, s.vEyeWide, s.tEyeWide, 0.12, 0.70); s.eyeWide = r.v; s.vEyeWide = r.vel; }
      { const r = spring(s.mouthCurve, s.vMouthCurve, s.tMouthCurve, 0.10, 0.78); s.mouthCurve = r.v; s.vMouthCurve = r.vel; }

      s.gait += dt * 0.012 * (0.4 + s.speed * 1.4) * (1 - s.levitate * 0.7);

      s.blinkTimer += dt;
      if (s.blinkTimer > 3200 + Math.random() * 2400) { s.blink = 1; s.blinkTimer = 0; }
      s.blink *= 0.82;

      const antTX = s.x + s.facing * 0.5, antTY = s.y - 56 - s.levitate * 4;
      s.antVx += (antTX - s.antX) * 0.05; s.antVx *= 0.82;
      s.antVy += (antTY - s.antY) * 0.05; s.antVy *= 0.82;
      s.antX += s.antVx; s.antY += s.antVy;
      const antTX2 = s.x + s.facing * 4, antTY2 = s.y - 50 - s.levitate * 4;
      s.antVx2 += (antTX2 - s.antX2) * 0.06; s.antVx2 *= 0.82;
      s.antVy2 += (antTY2 - s.antY2) * 0.06; s.antVy2 *= 0.82;
      s.antX2 += s.antVx2; s.antY2 += s.antVy2;

      if (s.fidget && s.phase === 'idle') {
        const maxMs = 900;
        s.fidgetValue = clamp(s.fidgetT / maxMs, 0, 1);
        if (s.fidgetT > maxMs) { s.fidget = null; s.fidgetT = 0; s.fidgetValue = 0; }
      } else { s.fidget = null; }

      const hbPhase = (now * 0.004) % (Math.PI * 2);
      const pulse = Math.max(0, Math.sin(hbPhase * 3)) * 0.9 + Math.sin(hbPhase) * 0.2 + s.attention * 0.4;
      s.hudWave[s.hudWaveHead] = pulse;
      s.hudWaveHead = (s.hudWaveHead + 1) % s.hudWave.length;

      for (const r of s.rings) if (r.alive) {
        r.r += (r.maxR - r.r) * 0.12; r.a *= 0.935;
        if (r.a < 0.02) r.alive = false;
      }
      for (const sp of s.sparks) if (sp.alive) {
        sp.x += sp.vx; sp.y += sp.vy;
        if (sp.kind === 'data') { sp.vy += -0.02; sp.vx *= 0.98; }
        else if (sp.kind === 'burst') { sp.vx *= 0.97; sp.vy *= 0.97; sp.vy += 0.02; }
        else { sp.vy += 0.04; sp.vx *= 0.95; }
      ...
      const crossShX = s.x - s.facing * 6, crossShY = s.y - 26;
      ...
      if (isDarkMode) { bg.addColorStop(0, '#070712'); bg.addColorStop(1, '#02020a'); }
      else { bg.addColorStop(0, '#f4f5f8'); bg.addColorStop(1, '#e7e8ee'); }
      ...
      drawFaceRing(ctx, s, photoX, photoY, photoW, photoH, landmarksRef.current, now);
      ...
      if (s.phase === 'scanning' || s.phase === 'analyzing' || s.phase === 'wireframe' || s.phase === 'summary') {
        drawHoloAccessories(ctx, s, now);
      }
      if (s.attention > 0.05 && s.primaryBeamOn) drawBeam(ctx, s.handX, s.handY, s.focusX, s.focusY, s.focusCol, s.attention, now, 1.0);
      if (s.crossBeamOn && s.attention > 0.2) drawBeam(ctx, s.crossHandX, s.crossHandY, s.crossTargetX, s.crossTargetY, '#ffffff', s.attention * 0.7, now, 0.55);
      drawDots(ctx, s, now);
...
function drawPhoto(ctx: CanvasRenderingContext2D, img: HTMLImageElement | null, x: number, y: number, w: number, h: number, isDarkMode: boolean, paraX: number) {
  ctx.save();
  ctx.shadowColor = 'rgba(0,0,0,0.45)'; ctx.shadowBlur = 24; ctx.shadowOffsetY = 10;
  rr(ctx, x, y, w, h, 20); ctx.fillStyle = '#000'; ctx.fill();
  ctx.restore();
  if (img) {
    ctx.save(); rr(ctx, x, y, w, h, 20); ctx.clip();
    const ir = img.width / img.height, pr = w / h;
    let dw = w, dh = h, dx = x + paraX, dy = y;
    if (ir > pr) { dw = h * ir; dx = x - (dw - w) / 2 + paraX; } else { dh = w / ir; dy = y - (dh - h) / 2; }
    ctx.drawImage(img, dx, dy, dw, dh); ctx.restore();
  }
  ctx.strokeStyle = isDarkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'; ctx.lineWidth = 1;
  rr(ctx, x, y, w, h, 20); ctx.stroke();
}

function drawGrid(ctx: CanvasRenderingContext2D, s: any, px: number, py: number, pw: number, ph: number, isDarkMode: boolean) {
  const g = s.grid; if (!g) return;
  const step = g.step;
  const rx = s.x, ry = s.y;
  const baseA = isDarkMode ? 0.07 : 0.09;
  const hotA = isDarkMode ? 0.28 : 0.30;
  const col = isDarkMode ? '#7dd3fc' : '#0ea5e9';
  ctx.save(); rr(ctx, px, py, pw, ph, 20); ctx.clip();
  ctx.lineWidth = 0.7;
  for (let i = 0; i < g.cols; i++) {
    const gx = px + i * step; const my = py + ph * 0.5;
    const dx = gx - rx, dy = my - ry; const dist = Math.hypot(dx, dy);
  ...
  for (let j = 0; j < g.rows; j++) {
    const gy = py + j * step; const mx = px + pw * 0.5;
    const dx = mx - rx, dy = gy - ry; const dist = Math.hypot(dx, dy);
...
function drawDistortion(ctx: CanvasRenderingContext2D, s: any, px: number, py: number, pw: number, ph: number) {
  ctx.save(); rr(ctx, px, py, pw, ph, 20); ctx.clip();
  ctx.globalCompositeOperation = 'lighter';
  const r = 120;
  const gr = ctx.createRadialGradient(s.x, s.y - 20, 0, s.x, s.y - 20, r);
  gr.addColorStop(0, hexA(s.focusCol, 0.10 * (0.5 + s.energy * 0.5)));
  gr.addColorStop(1, 'transparent');
  ctx.fillStyle = gr; ctx.fillRect(px, py, pw, ph);
  ctx.restore();
}

function drawFaceRing(ctx: CanvasRenderingContext2D, s: any, px: number, py: number, pw: number, ph: number, landmarks: Array<{ x: number; y: number }> | null | undefined, now: number) {
  if (!landmarks || landmarks.length < 400) return;
  let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  for (let i = 0; i < FACE_OVAL.length; i++) {
    const lm = landmarks[FACE_OVAL[i]];
    if (!lm) return;
    const x = px + lm.x * pw, y = py + lm.y * ph;
    _ovalX[i] = x; _ovalY[i] = y;
  ...
  const buildPath = () => {
    ctx.beginPath();
    const p0x = cx + (_ovalX[0] - cx) * breath, p0y = cy + (_ovalY[0] - cy) * breath;
    const p1x = cx + (_ovalX[1] - cx) * breath, p1y = cy + (_ovalY[1] - cy) * breath;
    ctx.moveTo((p0x + p1x) * 0.5, (p0y + p1y) * 0.5);
    for (let i = 1; i <= n; i++) {
      const curX = cx + (_ovalX[i % n] - cx) * breath, curY = cy + (_ovalY[i % n] - cy) * breath;
      const nxtX = cx + (_ovalX[(i + 1) % n] - cx) * breath, nxtY = cy + (_ovalY[(i + 1) % n] - cy) * breath;
      ctx.quadraticCurveTo(curX, curY, (curX + nxtX) * 0.5, (curY + nxtY) * 0.5);
    }
    ctx.closePath();
  };
  const col = '#7dd3fc', glow = '#38bdf8';
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
  ctx.strokeStyle = hexA(glow, 0.18 + s.energy * 0.12);
  ctx.lineWidth = 3.2; ctx.setLineDash([]);
  buildPath(); ctx.stroke();
  ctx.globalCompositeOperation = 'source-over';
  ctx.strokeStyle = hexA(col, 0.9); ctx.lineWidth = 1.1;
  ctx.setLineDash([2.2, 4]); ctx.lineDashOffset = -(now * 0.02) % 6.2;
  buildPath(); ctx.stroke();
  ctx.setLineDash([]); ctx.strokeStyle = hexA(col, 0.85); ctx.lineWidth = 1.3;
  const pad = 6;
  const bLx = minX - pad, bRx = maxX + pad, bTy = minY - pad, bBy = maxY + pad;
  const k = 10;
  ctx.beginPath(); ctx.moveTo(bLx + k, bTy); ctx.lineTo(bLx, bTy); ctx.lineTo(bLx, bTy + k); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(bRx - k, bTy); ctx.lineTo(bRx, bTy); ctx.lineTo(bRx, bTy + k); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(bLx + k, bBy); ctx.lineTo(bLx, bBy); ctx.lineTo(bLx, bBy - k); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(bRx - k, bBy); ctx.lineTo(bRx, bBy); ctx.lineTo(bRx, bBy - k); ctx.stroke();
  ctx.restore();
}
...
function drawThrusterWash(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ...
  gr.addColorStop(1, 'transparent');
  ctx.fillStyle = gr;
  ctx.beginPath(); ctx.arc(s.x, s.y + 14, r, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}
...
function drawRobot(ctx: CanvasRenderingContext2D, s: any, now: number, isDarkMode: boolean) {
  ...
  ctx.beginPath(); ctx.ellipse(bx, by + 4 + levitate * 6, shW, 3.2, 0, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
...
function drawThrusterRing(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ...
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  const ringR = 10, rot = now * 0.004;
  for (let i = 0; i < 6; i++) {
    const a = rot + (i / 6) * Math.PI * 2;
    const x = Math.cos(a) * ringR, y = -18 + Math.sin(a) * 3.5;
...
function drawLegs(ctx: CanvasRenderingContext2D, s: any, face: number, moving: boolean, levitate: number) {
  const UL = 12, LL = 13, hipY = -20, tuck = levitate;
  for (const dir of [-1, 1] as const) {
    const hx = dir * 5, hy = hipY;
    ...
    ctx.fillStyle = '#1e293b'; ctx.beginPath(); ctx.arc(hx, hy, 3, 0, Math.PI * 2); ctx.fill();
    ...
    ctx.fillStyle = '#0f172a'; rr(ctx, -3, -3, 6, 6, 1.5); ctx.fill();
    ctx.fillStyle = hexA(s.focusCol, 0.85);
    ctx.beginPath(); ctx.arc(0, 0, 1.3, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
    if (tuck < 0.9) {
      ctx.save(); ctx.globalAlpha = 1 - tuck * 0.8;
      ctx.translate(footX, footY); ctx.rotate(lift > 1 ? face * 0.22 : 0);
      ctx.fillStyle = '#020617';
      ctx.beginPath();
      ctx.moveTo(-5, 0); ctx.lineTo(9 * face, -1); ctx.lineTo(10 * face, 2.2); ctx.lineTo(-5, 2.5);
      ctx.closePath(); ctx.fill();
      const bootGr = ctx.createLinearGradient(0, -4, 0, 0);
      bootGr.addColorStop(0, '#64748b'); bootGr.addColorStop(1, '#1e293b');
      ctx.fillStyle = bootGr; rr(ctx, -4.5, -4, 12, 4, 1.5); ctx.fill();
      ctx.fillStyle = hexA(s.focusCol, 0.8); ctx.fillRect(-3.5, -0.8, 10, 0.9);
      ctx.strokeStyle = 'rgba(255,255,255,0.15)'; ctx.lineWidth = 0.5;
      for (let i = 0; i < 3; i++) { ctx.beginPath(); ctx.moveTo(-3 + i * 3, 0.5); ctx.lineTo(-3 + i * 3 + 1.5, 0.5); ctx.stroke(); }
      ctx.restore();
    }
  }
}

function drawMechLimb(ctx: CanvasRenderingContext2D, ax: number, ay: number, bx: number, by: number, thickness: number, accent: string) {
  const ang = Math.atan2(by - ay, bx - ax);
  const len = Math.hypot(bx - ax, by - ay);
  ctx.save();
  ctx.translate(ax, ay); ctx.rotate(ang);
  ctx.fillStyle = '#020617';
  rr(ctx, 0, -thickness / 2 - 0.5, len, thickness + 1, thickness / 2); ctx.fill();
  const gr = ctx.createLinearGradient(0, -thickness / 2, 0, thickness / 2);
  gr.addColorStop(0, '#f1f5f9'); gr.addColorStop(0.5, '#94a3b8'); gr.addColorStop(1, '#475569');
  ctx.fillStyle = gr;
  rr(ctx, 0.5, -thickness / 2, len - 1, thickness, thickness / 2 - 0.5); ctx.fill();
  ctx.strokeStyle = 'rgba(15,23,42,0.4)'; ctx.lineWidth = 0.6;
  ctx.beginPath(); ctx.moveTo(1, 0); ctx.lineTo(len - 1, 0); ctx.stroke();
  ctx.fillStyle = hexA(accent, 0.75);
  ctx.fillRect(len * 0.28, -thickness / 2 + 0.8, len * 0.42, 0.7);
  ctx.restore();
}

function drawTorso(ctx: CanvasRenderingContext2D, s: any, cx: number, cy: number, face: number, now: number, accent: string) {
  ctx.save();
  ctx.translate(cx, cy - 1);
  if (s.speed > 0.1) ctx.rotate(face * s.speed * 0.05);

  // Torso outline (premium: slightly larger for more HUD room)
  ctx.fillStyle = '#020617'; rr(ctx, -13, -17, 26, 24, 6.5); ctx.fill();
  ...
  ctx.fillStyle = tg; rr(ctx, -12, -16, 24, 22, 5.5); ctx.fill();
  // Central edge seam (vertical hairline)
  ctx.strokeStyle = 'rgba(8,12,20,0.35)'; ctx.lineWidth = 0.5;
  ctx.beginPath(); ctx.moveTo(0, -15); ctx.lineTo(0, 5); ctx.stroke();
  // Titanium trim horizontal line across upper chest
  ctx.strokeStyle = hexA('#d4af37', 0.7); ctx.lineWidth = 0.5;
  ctx.beginPath(); ctx.moveTo(-11, -11.5); ctx.lineTo(11, -11.5); ctx.stroke();
  // Dark HUD inset
  ctx.fillStyle = '#020617'; rr(ctx, -8, -14, 16, 17, 3); ctx.fill();

  // rotating tick-ring
  ctx.save();
  ctx.translate(0, -5); ctx.rotate(now * 0.002);
  ctx.strokeStyle = hexA(accent, 0.55); ctx.lineWidth = 0.6;
  for (let i = 0; i < 6; i++) {
  ...
  cg.addColorStop(0, hexA(accent, pulse)); cg.addColorStop(1, 'transparent');
  ctx.fillStyle = cg; ctx.beginPath(); ctx.arc(0, -5, 10, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
  ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(0, -5, 2.2, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = accent; ctx.beginPath(); ctx.arc(0, -5, 1.3, 0, Math.PI * 2); ctx.fill();

  // HUD row 1: state text
  ctx.save();
  ctx.beginPath(); rr(ctx, -7.5, -13.5, 15, 3.2, 0.8); ctx.clip();
  ctx.fillStyle = '#050910'; ctx.fillRect(-7.5, -13.5, 15, 3.2);
  ...
  ctx.save();
  ctx.beginPath(); rr(ctx, -7.5, -9.8, 15, 3.4, 0.6); ctx.clip();
  ctx.fillStyle = '#050910'; ctx.fillRect(-7.5, -9.8, 15, 3.4);
  ctx.strokeStyle = hexA(accent, 0.9); ctx.lineWidth = 0.6;
  ctx.beginPath();
  const wave: Float32Array = s.hudWave; const head: number = s.hudWaveHead;
  ...
  // HUD row 3: bars
  ctx.save();
  ctx.beginPath(); rr(ctx, -7.5, -6.0, 15, 2.0, 0.5); ctx.clip();
  ctx.fillStyle = '#050910'; ctx.fillRect(-7.5, -6.0, 15, 2.0);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    ctx.strokeStyle = hexA(accent, 0.3); ctx.lineWidth = 0.3;
  ...
  for (const dir of [-1, 1] as const) {
    ctx.save(); ctx.translate(dir * 11, -13);
    // underplate
    ctx.fillStyle = '#0b1220';
    ctx.beginPath();
    ctx.moveTo(-3.5, 1.5); ctx.lineTo(-3.5, -2.5);
    ctx.quadraticCurveTo(-3.5, -4.5, 0, -4.5);
    ctx.quadraticCurveTo(3.5, -4.5, 3.5, -2.5);
    ctx.lineTo(3.5, 1.5); ctx.closePath(); ctx.fill();
    // chrome armor overplate
    const pg = ctx.createLinearGradient(0, -4, 0, 1);
    pg.addColorStop(0, '#f1f5f9'); pg.addColorStop(0.55, '#8b93a1'); pg.addColorStop(1, '#334155');
    ctx.fillStyle = pg;
    ctx.beginPath();
    ctx.moveTo(-2.8, 0.8); ctx.lineTo(-2.8, -2.2);
    ctx.quadraticCurveTo(-2.8, -3.9, 0, -3.9);
    ctx.quadraticCurveTo(2.8, -3.9, 2.8, -2.2);
    ctx.lineTo(2.8, 0.8); ctx.closePath(); ctx.fill();
    // titanium trim edge
    ctx.strokeStyle = hexA('#d4af37', 0.8); ctx.lineWidth = 0.4;
    ctx.beginPath(); ctx.arc(0, -2.2, 2.8, Math.PI, 0); ctx.stroke();
    // Accent LED
    const isBeamSide = (dir === s.facing && s.primaryBeamOn) || (dir !== s.facing && s.crossBeamOn);
    ctx.fillStyle = hexA(accent, isBeamSide ? 0.95 : 0.4);
    ctx.beginPath(); ctx.arc(0, -2, 0.85, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }
  // Side heat vents
  for (const dir of [-1, 1] as const) {
    ctx.save(); ctx.translate(dir * 11, -4);
...
function drawArm(ctx: CanvasRenderingContext2D, s: any, _cx: number, hipY: number, side: -1 | 1, _now: number, accent: string, isBeamArm: boolean) {
  const shoulderX = side * 6, shoulderY = hipY;
  let handTX: number, handTY: number;
  // Finale pose overrides take priority
  if (s.finaleStage === 1) {
    // V-pose: both arms raised outward
    handTX = side * 18; handTY = hipY - 18;
  } else if (s.finaleStage === 2 || s.finaleStage === 3) {
    // Salute: dominant-facing arm to temple, other at side
    if (side === s.facing) { handTX = side * 5; handTY = hipY - 20; }
    else { handTX = side * 10; handTY = hipY + 6; }
  } else if (!isBeamArm && s.fidget === 'stretch' && side === -s.facing) {
    const v = s.fidgetValue; const raised = Math.sin(v * Math.PI) * 20;
    handTX = -s.facing * 14; handTY = hipY - 4 - raised;
  } else if (!isBeamArm && s.fidget === 'checkWrist' && side === -s.facing) {
    handTX = -s.facing * 4; handTY = hipY - 8;
  } else if (isBeamArm) {
    handTX = s.handX - s.x; handTY = s.handY - s.y;
  } else if (!isBeamArm && s.crossBeamOn) {
    handTX = s.crossHandX - s.x; handTY = s.crossHandY - s.y;
  } else {
    const swing = Math.sin(s.gait + (side > 0 ? Math.PI : 0)) * (s.speed > 0.1 ? 6 : 1.5);
    handTX = side * 11; handTY = hipY + 16 + swing;
  }
  const UA = 10, FA = 11;
  const elbow = ik2(shoulderX, shoulderY, handTX, handTY, UA, FA, -side);
  ctx.fillStyle = '#1e293b'; ctx.beginPath(); ctx.arc(shoulderX, shoulderY, 3, 0, Math.PI * 2); ctx.fill();
  drawMechLimb(ctx, shoulderX, shoulderY, elbow.x, elbow.y, 5.4, accent);
  drawMechLimb(ctx, elbow.x, elbow.y, handTX, handTY, 4.8, accent);
  ctx.fillStyle = '#0f172a'; ctx.beginPath(); ctx.arc(elbow.x, elbow.y, 2.2, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = hexA(accent, 0.85); ctx.beginPath(); ctx.arc(elbow.x, elbow.y, 0.7, 0, Math.PI * 2); ctx.fill();

  ctx.save();
  ctx.translate(handTX, handTY);
  ctx.rotate(Math.atan2(handTY - elbow.y, handTX - elbow.x));
  ctx.fillStyle = '#0f172a'; rr(ctx, -2, -3.2, 6.5, 6.4, 1.5); ctx.fill();
  if (isBeamArm) {
    const spread = s.armExtend;
    ctx.strokeStyle = '#64748b'; ctx.lineWidth = 0.8;
    for (let i = 0; i < 3; i++) {
      const offs = (i - 1) * 0.6 * spread;
      ctx.beginPath(); ctx.moveTo(4, 0); ctx.lineTo(4 + 3 * spread, offs * 2.2); ctx.stroke();
    }
    if (s.armExtend > 0.1) {
      ctx.save(); ctx.globalCompositeOperation = 'lighter';
      const pulse = 0.6 + s.energy * 0.4 + (s.primaryBeamOn ? 0.3 : 0);
      const gr = ctx.createRadialGradient(4, 0, 0, 4, 0, 7);
      gr.addColorStop(0, hexA(accent, pulse)); gr.addColorStop(1, 'transparent');
      ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(4, 0, 7, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    }
  } else {
    ctx.fillStyle = '#334155'; rr(ctx, -1.5, -2.5, 4, 5, 1.2); ctx.fill();
    if (s.crossBeamOn) {
      ctx.save(); ctx.globalCompositeOperation = 'lighter';
      const gr = ctx.createRadialGradient(2, 0, 0, 2, 0, 5);
      gr.addColorStop(0, hexA('#ffffff', 0.8)); gr.addColorStop(1, 'transparent');
      ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(2, 0, 5, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    }
  }
  ctx.restore();
...
function drawHead(ctx: CanvasRenderingContext2D, s: any, cx: number, cy: number, face: number, now: number, accent: string, _isDarkMode: boolean) {
  ctx.save();
  ctx.translate(cx, cy);
  ...
  ctx.fillStyle = '#0b1220'; rr(ctx, -3.5, 0, 7, 5, 1.4); ctx.fill();
  ctx.strokeStyle = hexA(accent, 0.3 + s.blink * 0.7); ctx.lineWidth = 0.4;
  ctx.beginPath(); ctx.moveTo(-3, 1.6); ctx.lineTo(3, 1.6); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(-3, 3.3); ctx.lineTo(3, 3.3); ctx.stroke();

  // Head shell
  ctx.fillStyle = '#020617'; rr(ctx, -12, -14, 24, 18, 6); ctx.fill();
  ...
  ctx.strokeStyle = hexA('#d4af37', 0.85); ctx.lineWidth = 0.5;
  ctx.beginPath(); ctx.moveTo(-2.5, -16); ctx.lineTo(2.5, -16); ctx.stroke();
  ctx.restore();

  // Edge seam down the middle of head
  ctx.strokeStyle = 'rgba(8,12,20,0.3)'; ctx.lineWidth = 0.4;
  ctx.beginPath(); ctx.moveTo(0, -12); ctx.lineTo(0, -10); ctx.stroke();

  for (const dir of [-1, 1] as const) {
    ctx.save(); ctx.translate(dir * 11, -5);
  ...
  ctx.fillStyle = '#020617'; rr(ctx, -9, -9, 18, 8, 2.5); ctx.fill();
  ctx.save();
  ctx.beginPath(); rr(ctx, -9, -9, 18, 8, 2.5); ctx.clip();
  ...
  ctx.fillStyle = sg; ctx.fillRect(-10, -10, 20, 10);
  ctx.restore();

  // brows
  ctx.strokeStyle = hexA(accent, 0.9); ctx.lineWidth = 1.2;
  ...
  for (const ex of [-4, 4]) {
    ...
    gr.addColorStop(0, hexA(accent, 0.95)); gr.addColorStop(1, 'transparent');
    ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(ex + ox, eyeBaseY + oy, 4.5, 0, Math.PI * 2); ctx.fill();
  }
  ctx.restore();

  for (const ex of [-4, 4]) {
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.ellipse(ex + ox, eyeBaseY + oy, eyeW * 0.5, eyeH * 0.5, 0, 0, Math.PI * 2); ctx.fill();
    if (s.crossBeamOn) {
      ctx.fillStyle = accent;
      ctx.beginPath(); ctx.arc(ex + ox - 0.6, eyeBaseY + oy, 0.5, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(ex + ox + 0.6, eyeBaseY + oy, 0.5, 0, Math.PI * 2); ctx.fill();
    } else {
      ctx.fillStyle = accent;
      ctx.beginPath(); ctx.ellipse(ex + ox, eyeBaseY + oy, Math.min(eyeW * 0.25, 0.8), Math.min(eyeH * 0.55, 0.9), 0, 0, Math.PI * 2); ctx.fill();
    }
  }

  if (Math.abs(s.mouthCurve) > 0.05) {
    ctx.strokeStyle = hexA(accent, 0.75); ctx.lineWidth = 0.9;
    ctx.beginPath();
  ...
  ctx.beginPath(); ctx.moveTo(0, -13); ctx.quadraticCurveTo(ax * 0.3, -20, ax, ay); ctx.stroke();
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  const ap = 0.5 + s.energy * 0.5;
  const ag = ctx.createRadialGradient(ax, ay, 0, ax, ay, 4);
  ag.addColorStop(0, hexA(accent, ap)); ag.addColorStop(1, 'transparent');
  ctx.fillStyle = ag; ctx.beginPath(); ctx.arc(ax, ay, 4, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
  ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(ax, ay, 1.1, 0, Math.PI * 2); ctx.fill();

  const ax2 = s.antX2 - s.x - face * 2;
  const ay2 = s.antY2 - s.y - cy;
  ctx.strokeStyle = '#475569'; ctx.lineWidth = 0.9;
  ctx.beginPath(); ctx.moveTo(face * 4, -11); ctx.quadraticCurveTo(ax2 * 0.5, -16, ax2, ay2); ctx.stroke();
  const blink2 = 0.3 + Math.sin(now * 0.01 + 1.7) * 0.3 + (s.crossBeamOn ? 0.4 : 0);
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  const ag2 = ctx.createRadialGradient(ax2, ay2, 0, ax2, ay2, 2.8);
  ag2.addColorStop(0, hexA('#22d3ee', clamp(blink2, 0, 1))); ag2.addColorStop(1, 'transparent');
  ctx.fillStyle = ag2; ctx.beginPath(); ctx.arc(ax2, ay2, 2.8, 0, Math.PI * 2); ctx.fill();
  ctx.restore();

  ctx.restore();
}

function drawBeam(ctx: CanvasRenderingContext2D, ox: number, oy: number, tx: number, ty: number, accent: string, attention: number, now: number, intensity: number) {
  ...
  ctx.strokeStyle = hexA(accent, alpha * 0.35); ctx.lineWidth = width * 2.2;
  ctx.beginPath(); ctx.moveTo(ox, oy); ctx.lineTo(tx, ty); ctx.stroke();
  ctx.strokeStyle = hexA(accent, alpha * 0.7); ctx.lineWidth = width;
  ctx.beginPath(); ctx.moveTo(ox, oy); ctx.lineTo(tx, ty); ctx.stroke();
  ctx.strokeStyle = hexA('#ffffff', alpha); ctx.lineWidth = Math.max(1.2, width * 0.3);
  ctx.beginPath(); ctx.moveTo(ox, oy); ctx.lineTo(tx, ty); ctx.stroke();
  const tg = ctx.createRadialGradient(tx, ty, 0, tx, ty, 14);
  tg.addColorStop(0, hexA(accent, alpha)); tg.addColorStop(1, 'transparent');
  ctx.fillStyle = tg; ctx.beginPath(); ctx.arc(tx, ty, 14, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}

function drawDots(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  for (const d of s.dots) if (d.alive) {
    ...
    gr.addColorStop(0, hexA(d.col, 0.6)); gr.addColorStop(1, 'transparent');
    ctx.fillStyle = gr;
    ctx.beginPath(); ctx.arc(d.x, d.y, r * 2.4, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = hexA('#ffffff', 0.9);
    ctx.beginPath(); ctx.arc(d.x, d.y, Math.max(1.2, r * 0.5), 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = d.col;
    ctx.beginPath(); ctx.arc(d.x, d.y, Math.max(0.8, r * 0.3), 0, Math.PI * 2); ctx.fill();
  }
  ctx.restore();
}
...
function drawWireframes(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ...
  for (const w of s.wireframes as Wireframe[]) {
    ...
    if (w.prog < 1) {
      ...
      let lo = 0, hi = w.count - 1;
      while (lo < hi) {
        const m = (lo + hi) >> 1;
        if (w.cumLen[m] < visible) lo = m + 1; else hi = m;
      }
      const hx = w.pts[lo * 2], hy = w.pts[lo * 2 + 1];
      ctx.setLineDash([]);
...
function drawGlowPass(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  for (const r of s.rings) if (r.alive) {
    ctx.strokeStyle = hexA(r.col, r.a); ctx.lineWidth = r.w;
  ...
  for (const sp of s.sparks) if (sp.alive) {
    if (sp.kind === 'data') {
      ...
      gr.addColorStop(1, 'transparent');
      ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(sp.x, sp.y, r, 0, Math.PI * 2); ctx.fill();
    } else {
      const gr = ctx.createRadialGradient(sp.x, sp.y, 0, sp.x, sp.y, 3);
      gr.addColorStop(0, hexA(sp.col, sp.life * 0.9)); gr.addColorStop(1, 'transparent');
      ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(sp.x, sp.y, 3, 0, Math.PI * 2); ctx.fill();
    }
  }
  // Focus hotspot bloom
  if (s.attention > 0.05) {
    ...
    gr.addColorStop(1, 'transparent');
    ctx.fillStyle = gr;
    ctx.beginPath(); ctx.arc(s.focusX, s.focusY, 22, 0, Math.PI * 2); ctx.fill();
  }
  ctx.restore();
  void now;
}
...
function drawHoloAccessories(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.fillStyle = hexA('#7dd3fc', alpha * (0.6 + 0.4 * ((i + Math.floor(now * 0.004)) % 3) / 2));
  ...
  const drawBadge = (bxp: number, byp: number, text: string) => {
    ...
    rr(ctx, bxp - w / 2, byp - h / 2, w, h, 1.5); ctx.fill();
    ctx.strokeStyle = hexA('#7dd3fc', 0.9 * alpha); ctx.lineWidth = 0.6;
    rr(ctx, bxp - w / 2, byp - h / 2, w, h, 1.5); ctx.stroke();
  ...
  gr.addColorStop(1, 'transparent');
  ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(probeX, probeY, 4, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = hexA('#ffffff', 0.95 * alpha);
  ctx.beginPath(); ctx.arc(probeX, probeY, 0.9, 0, Math.PI * 2); ctx.fill();
  // short emitter line pointing along velocity tangent
  const tanX = -Math.sin(probeT) * rx, tanY = Math.cos(probeT) * ry;
...
function drawDoneHolo(ctx: CanvasRenderingContext2D, s: any, now: number) {
  ...
  gr.addColorStop(1, 'transparent');
  ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(0, 0, 40 * scale, 0, Math.PI * 2); ctx.fill();
  ctx.globalCompositeOperation = 'source-over';
  // Glass card
  ...
  rr(ctx, -w / 2 + 1.5, -h / 2 + 1.5, w - 3, (h - 3) * 0.4, 3); ctx.fill();
  // Text
  ctx.fillStyle = '#ffffff';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
