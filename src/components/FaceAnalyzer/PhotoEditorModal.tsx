...
export function PhotoEditorModal({ imageUrl, isDarkMode, onConfirm, onCancel }: Props) {
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    switch (dragState.current.handle) {
      ...
      case 'nw':
        ...
        nw = sc.x + sc.w - nx; nh = sc.y + sc.h - ny;
        break;
      case 'ne':
        ny = sc.y + dy;
        nw = sc.w + dx; nh = sc.y + sc.h - ny;
        break;
      case 'sw':
        nx = sc.x + dx;
        nw = sc.x + sc.w - nx; nh = sc.h + dy;
        break;
      case 'se':
  ...
  const handleReset = () => {
    setCoarseRot(0); setFineRot(0); setFlipH(false);
    ...
    >
      ...
      <motion.div
        className={cn(
          ...
          isDarkMode ? 'bg-[#111114] border border-white/[0.06]' : 'bg-white border border-zinc-200/80'
        )}
      ...
      >
        {/* ── Header ── */}
        <div className={cn("flex items-center justify-between px-5 py-3.5 border-b", isDarkMode ? 'border-white/5' : 'border-zinc-100')}>
          <div className="flex items-center gap-3">
            <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center", isDarkMode ? 'bg-indigo-500/15' : 'bg-indigo-50')}>
            ...
            <div>
              <p className={cn("text-sm font-semibold leading-none", isDarkMode ? 'text-white' : 'text-zinc-900')}>Adjust Photo</p>
              <p className={cn("text-[11px] mt-0.5", isDarkMode ? 'text-white/30' : 'text-zinc-400')}>Crop & rotate before analysis</p>
            </div>
          </div>
          <button
            onClick={onCancel}
            className={cn("w-7 h-7 flex items-center justify-center rounded-full transition-colors", isDarkMode ? 'hover:bg-white/8 text-white/40' : 'hover:bg-zinc-100 text-zinc-400')}
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className={cn("p-4", isDarkMode ? 'bg-[#0a0a0d]' : 'bg-zinc-50')}>
          ...
          >
            ...
            >
              ...
              >
                ...
                {[1/3, 2/3].map(t => (
                  <React.Fragment key={t}>
                    <div className="absolute w-full" style={{ height: 1, top: `${t * 100}%`, background: 'rgba(255,255,255,0.13)' }} />
                    <div className="absolute h-full" style={{ width: 1, left: `${t * 100}%`, background: 'rgba(255,255,255,0.13)' }} />
                  </React.Fragment>
                ))}
              </div>
              ...
              {([
                { id: 'nw', cx: x,     cy: y,     t: 'translate(-1px,-1px)',        bl: '0 0 0 0', bw: '2.5px 0 0 2.5px' },
                { id: 'ne', cx: x + w, cy: y,     t: 'translate(calc(-100% + 1px),-1px)', bl: '0 0 0 0', bw: '2.5px 2.5px 0 0' },
                { id: 'sw', cx: x,     cy: y + h, t: 'translate(-1px,calc(-100% + 1px))', bl: '0 0 0 0', bw: '0 0 2.5px 2.5px' },
                { id: 'se', cx: x + w, cy: y + h, t: 'translate(calc(-100% + 1px),calc(-100% + 1px))', bl: '0 0 0 0', bw: '0 2.5px 2.5px 0' },
              ] as const).map(({ id, cx: hx, cy: hy, t: transform, bw }) => (
                <div
                  key={id}
              ...
              <div
                className="absolute pointer-events-auto"
                style={{
                  left: `${(x + 0.08) * 100}%`, top: `${(y + 0.08) * 100}%`,
                  width: `${(w - 0.16) * 100}%`, height: `${(h - 0.16) * 100}%`,
                  cursor: 'move',
                  zIndex: 12,
                }}
              />
        ...
        {/* ── Rotation controls ── */}
        <div className={cn("px-5 pt-3 pb-2 space-y-3 border-t", isDarkMode ? 'border-white/5' : 'border-zinc-100')}>
          <div className="flex items-center gap-2.5">
            {/* Coarse rotate buttons */}
            <button
              onClick={() => setCoarseRot(r => r - 90)}
              className={cn("w-8 h-8 flex items-center justify-center rounded-lg transition-colors text-sm font-medium", isDarkMode ? 'bg-white/5 hover:bg-white/10 text-white/60' : 'bg-zinc-100 hover:bg-zinc-200 text-zinc-600')}
              title="Rotate −90°"
            >
            ...
            <button
              onClick={() => setCoarseRot(r => r + 90)}
              className={cn("w-8 h-8 flex items-center justify-center rounded-lg transition-colors", isDarkMode ? 'bg-white/5 hover:bg-white/10 text-white/60' : 'bg-zinc-100 hover:bg-zinc-200 text-zinc-600')}
              title="Rotate +90°"
            >
            ...
            <button
              ...
              className={cn(
                ...
                flipH
                  ? (isDarkMode ? 'bg-indigo-500/25 text-indigo-400' : 'bg-indigo-100 text-indigo-600')
                  : (isDarkMode ? 'bg-white/5 hover:bg-white/10 text-white/60' : 'bg-zinc-100 hover:bg-zinc-200 text-zinc-600')
              )}
            ...
            {(coarseRot !== 0 || fineRot !== 0 || flipH) && (
              <button
                onClick={handleReset}
                className={cn("flex items-center gap-1.5 text-xs font-medium transition-colors", isDarkMode ? 'text-white/30 hover:text-white/60' : 'text-zinc-400 hover:text-zinc-600')}
              >
            ...
            {totalRot !== 0 && (
              <span className={cn("text-[11px] tabular-nums font-mono w-10 text-right", isDarkMode ? 'text-white/30' : 'text-zinc-400')}>
          ...
          <div className="flex items-center gap-3">
            <span className={cn("text-[9px] font-bold uppercase tracking-widest shrink-0", isDarkMode ? 'text-white/20' : 'text-zinc-400')}>Fine</span>
            <input
              type="range"
              min={-15}
              max={15}
            ...
            />
            <span className={cn("text-[10px] tabular-nums font-mono w-9 text-right shrink-0", isDarkMode ? 'text-white/25' : 'text-zinc-400')}>
        ...
        {/* ── Footer buttons ── */}
        <div className={cn("px-5 py-4 flex items-center gap-3 border-t", isDarkMode ? 'border-white/5' : 'border-zinc-100')}>
          <button
            onClick={onCancel}
            className={cn(
              "flex-1 py-2.5 rounded-xl text-sm font-medium border transition-colors",
              isDarkMode ? 'border-white/8 text-white/40 hover:bg-white/5 hover:text-white/60' : 'border-zinc-200 text-zinc-500 hover:bg-zinc-50'
            )}
          >
            Cancel
          </button>
