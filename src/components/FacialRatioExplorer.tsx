...
export function FacialRatioExplorer({
  ...
  const { scrollToPricing, onOpenPricing } = useDashboardContext();
  const [activeIndex, setActiveIndex] = useState(0);

  const sectionRef = useRef<HTMLDivElement>(null);
  const leftCardRef = useRef<HTMLDivElement>(null);
  // One-shot snap latch: once the face card has been centered in the
  // viewport, we stop fighting the user. Reset only when the section
  // leaves the viewport so the next visit re-snaps.
  const snappedRef = useRef(false);

  // Re-arm the snap whenever the section scrolls fully out of view.
  useEffect(() => {
    const sec = sectionRef.current;
    if (!sec) return;
    const obs = new IntersectionObserver(
      ([entry]) => {
        if (!entry.isIntersecting) snappedRef.current = false;
      },
      { threshold: 0 }
    );
    obs.observe(sec);
  ...
  const centerFaceCard = useCallback(() => {
  ...
  // Wheel inside the ratio list:
  //  - First wheel tick after entering the section snap-centers the face
  //    card in the viewport (smooth, one-shot).
    ...
    () =>
      [
        ...RATIO_GROUPS.map((g) => ({
          ...
          items: ratios
            .map((r, idx) => ({ r, idx }))
            .filter(({ r }) => (RATIO_GROUP_MAP[r.shortName] ?? 'Other') === g.name)
        })),
        {
          name: 'Other',
          icon: '📐',
          items: ratios
            .map((r, idx) => ({ r, idx }))
            .filter(({ r }) => !RATIO_GROUP_MAP[r.shortName])
        }
  ...
  const zoomFocus = useMemo(() => {
    ...
    const validPts = primaryIndices.map((i) => points[i]).filter(Boolean) as {
      x: number;
      y: number;
    }[];
  ...
  const idealStartPct = Math.max(
    0,
    Math.min(100, ((active.idealRange[0] - scaleMin) / scaleRange) * 100)
  );
  const idealEndPct = Math.max(
    0,
    Math.min(100, ((active.idealRange[1] - scaleMin) / scaleRange) * 100)
  );
  ...
  const renderPartiallyBlurredText = (text: string, visibleChunkSize = 4, hiddenChunkSize = 3) => {
    ...
    return words.map((word, idx) => {
      ...
      const isVisible = indexInCycle < visibleChunkSize;
      return (
        <React.Fragment key={`${word}-${idx}`}>
          <span
            className={cn(
              'inline',
              isVisible ? '' : 'blur-[2px] opacity-70 select-none pointer-events-none'
            )}
          >
            {word}
          </span>
  ...
  return (
    <div ref={sectionRef}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p
              className={cn('text-[10px] opacity-40', isDarkMode ? 'text-white' : 'text-zinc-900')}
            >
        ...
        <div className="hidden md:flex flex-col gap-1.5 min-w-0 max-w-[42%]">
          <div className="flex items-center justify-between gap-3">
            <span
              className={cn(
                'text-[8px] font-bold uppercase tracking-widest opacity-35 whitespace-nowrap',
            ...
            <span
              className={cn(
                'text-[8px] font-bold tabular-nums whitespace-nowrap',
                getScoreColor(avgScore).text
              )}
            >
        ...
        >
          <div className="relative overflow-hidden">
            ...
            >
              ...
              {/* Dark overlay to make lines more visible */}
                      ...
                      <div className="flex items-baseline gap-1">
                        <span
                          className={cn(
                            ...
                            isLocked ? 'text-zinc-500' : getScoreColor(display.score).text
                          )}
                        >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={cn(
                          ...
                          isDarkMode ? 'bg-white/6 text-white/22' : 'bg-zinc-100 text-zinc-400'
                        )}
                      >
                        {group.items.length}
                      </span>
                          ...
                          className={cn(
                            ...
                            isDarkMode
                              ...
                              : isActive
                                ...
                                : 'bg-transparent border-zinc-100/60 hover:bg-zinc-50/80 hover:border-zinc-200'
                          )}
                        >
                          {/* Left accent dot */}
                          <motion.div
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              {rIsWithin && !isLocked && (
                                <span
                                  className={cn(
                                    'text-[8px] font-semibold tracking-wide uppercase flex-shrink-0',
                          ...
                          >
                            ...
                            >
                              ...
                              {(() => {
                                const idealS = Math.max(
                                  0,
                                  Math.min(
                                    100,
                                    ((r.idealRange[0] - r.idealRange[0] * 0.6) /
                                      (r.idealRange[1] * 1.5 - r.idealRange[0] * 0.6)) *
                                      100
                                  )
                                );
                                const idealE = Math.max(
                                  0,
                                  Math.min(
                                    100,
                                    ((r.idealRange[1] - r.idealRange[0] * 0.6) /
                                      (r.idealRange[1] * 1.5 - r.idealRange[0] * 0.6)) *
                                      100
                                  )
                                );
                          ...
                          {/* Right: Score / unlock action */}
                          <div className="w-[92px] md:w-[106px] flex-shrink-0 flex items-center justify-end">
                            {isLocked ? (
                              <button
                                type="button"
                                onClick={(e) => {
                                ...
                                className={cn(
                                  'group/unlock inline-flex h-6.5 items-center gap-1.5 rounded-lg border px-2.5 text-[9px] font-black uppercase tracking-[0.16em] transition-all duration-200',
                                  isDarkMode
                                    ? 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06] hover:border-cyan-400/30'
                                    : 'border-zinc-200 bg-white hover:bg-zinc-50 hover:border-cyan-300'
                                )}
                              >
                                <Lock
                                  className={cn(
                                    ...
                                    isDarkMode
                                      ? 'text-zinc-400 group-hover/unlock:text-cyan-300'
                                      : 'text-zinc-400 group-hover/unlock:text-cyan-600'
                                  )}
                                />
                                <span
                                  className={cn(
      ...
      {/* ── Detail Modal (portaled to <body> so transformed ancestors don't break position:fixed) ── */}
      {createPortal(
        <AnimatePresence>
          {modalOpen &&
            (() => {
              ...
              const moIdealS = Math.max(
                0,
                Math.min(100, ((mo.idealRange[0] - moMin) / moRange) * 100)
              );
              const moIdealE = Math.max(
                0,
                Math.min(100, ((mo.idealRange[1] - moMin) / moRange) * 100)
              );
              ...
              const mayIndicateMap: Record<string, string[]> = {
                fWHR: [
                  'Perceived dominance & authority',
                  'Testosterone exposure level',
                  'Facial masculinity index'
                ],
                'φ Ratio': [
                  'Classical beauty alignment',
                  'Structural facial balance',
                  'Natural proportion harmony'
                ],
                Symmetry: [
                  'Genetic health & stability',
                  'Developmental quality',
                  'Baseline attractiveness factor'
                ],
                'Canthal Tilt': [
                  'Youthfulness & alertness',
                  'Eye attractiveness level',
                  'Upper-face aesthetic appeal'
                ],
                'Jaw Angle': [
                  'Jawline definition & sharpness',
                  'Facial masculinity',
                  'Lower-face structure quality'
                ],
                'Eye Shape': [
                  'Eye shape attractiveness',
                  'Almond-eye quality',
                  'Periorbital aesthetics'
                ],
                Thirds: [
                  'Proportional face balance',
                  'Classical beauty standard',
                  'Vertical facial harmony'
                ],
                'Eye Spacing': [
                  'Interpupillary harmony',
                  'Classical one-eye-width rule',
                  'Orbital spacing quality'
                ],
                'Eye/Face W': [
                  'Eye prominence & frame',
                  'Facial width harmony',
                  'Upper-face proportional balance'
                ],
                'Jaw/Cheek': [
                  'V-line face structure',
                  'Facial tapering quality',
                  'Cheekbone-to-jaw definition'
                ],
                'Nose/Mouth': [
                  'Nasal proportion balance',
                  'Lower-face harmony',
                  'Golden-ratio conformance'
                ],
                'Mouth/Face': [
                ...
                Philtrum: [
                  ...
                  'Aging progression marker',
                  'Upper-lip proportion'
                ],
                'Lip Ratio': [
                  'Lip fullness & attractiveness',
                  'Youthful lip proportion',
                  'Sensuality perception'
                ],
                'Lower Face': [
                  'Chin projection adequacy',
                  'Facial maturity level',
                  'Lower-third balance'
                ],
                Midface: [
                  'Midface elongation level',
                  'Nasal-bridge proportion',
                  'Central facial aesthetics'
                ],
                Intercanthal: [
                  'Orbital spacing harmony',
                  'Neoclassical alignment to alar base',
                  'Close-set vs wide-set eye classification'
                ],
                'Nasal Index': [
                  'Nasal width-to-length proportion',
                  'Rhinoplasty candidacy assessment',
                  'Ethnic & neoclassical canon adherence'
                ],
                'Chin Height': [
                  'Chin projection & vertical balance',
                  'Genioplasty or chin implant need',
                  'Lower-face maturity & age perception'
                ],
                Forehead: [
                  'Upper-face elongation level',
                  'Hairline and brow position balance',
                  'Vertical facial thirds harmony'
                ],
                'Jaw/Height': [
                  'Jaw strength relative to face height',
                  'Mandibular width-to-height balance',
                  'Lower-face structural dominance'
                ],
                'Nose/Eye W': [
                  'Neoclassical one-eye-width nasal rule',
                  'Nasal alar width correction need',
                  'Orbital frame vs nasal width balance'
                ],
                'Face Index': [
                  'Mesoprosopic vs euryprosopic classification',
                  'Overall face shape proportionality',
                  'Width-to-height harmony index'
                ],
                'Eye Open': [
                  'Palpebral fissure vertical aperture',
                  'Hooded eye or ptosis indicator',
                  'Hunter-eye vs round-eye morphology'
                ],
                'Brow Lift': [
                  'Brow ptosis or over-elevation level',
                  'Brow lift procedure candidacy',
                  'Upper-face alertness & expression perception'
                ],
                'Bridge/Alar': [
                  'Nasal bridge-to-base taper ratio',
                  'Rhinoplasty dorsum width target',
                  'Narrow vs wide dorsal aesthetic'
                ]
              };
              const moIndicators = mayIndicateMap[mo.shortName] ?? [];

              const scoreColor = moWithin ? '#22c55e' : mo.score >= 6 ? '#f97316' : '#ef4444';
              const scoreGlow = moWithin
                ? 'rgba(34,197,94,0.4)'
                : mo.score >= 6
                  ? 'rgba(249,115,22,0.4)'
                  : 'rgba(239,68,68,0.4)';
                          ...
                          <div className="flex items-end justify-between">
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span
                                  className={cn(
                                    'text-[9px] font-medium',
                                    isDarkMode ? 'text-white/30' : 'text-white/40'
                                  )}
                                >
                            ...
                            >
                              <div className="flex items-baseline gap-1 relative">
                                ...
                                <span
                                  className={cn(
                                    'text-4xl md:text-5xl font-bold tabular-nums leading-none',
                                    moSc.text
                                  )}
                                >
                    ...
                    >
                      ...
                      <motion.div
                        className={cn(
                          ...
                          isDarkMode ? 'border-white/6' : 'border-zinc-100'
                        )}
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <motion.button
                            ...
                            onClick={() =>
                              setActiveIndex((i) => Math.min(ratios.length - 1, i + 1))
                            }
                          >
                      ...
                      <div
                        className={cn(
                          'grid grid-cols-4 gap-px flex-shrink-0 border-b',
                          isDarkMode ? 'bg-white/8 border-white/6' : 'bg-zinc-100 border-zinc-100'
                        )}
                      >
                        {[
                          {
                          ...
                          {
                            label: 'IDEAL RANGE',
                            value: `${mo.idealRange[0]}–${mo.idealRange[1]}${mo.unit ?? ''}`,
                            sub: `Mid: ${moMid.toFixed(2)}`,
                            color: isDarkMode ? 'text-emerald-400' : 'text-emerald-700'
                          },
                          {
                            label: 'SCORE',
                            value: `${mo.score.toFixed(1)}`,
                            sub: '/10',
                            color: moSc.text
                          },
                          {
                          ...
                          >
                            <p
                              className={cn(
                                'text-[7px] font-black uppercase tracking-widest mb-1.5 opacity-30',
                                isDarkMode ? 'text-white' : 'text-zinc-900'
                              )}
                            >
                              {s.label}
                            ...
                            {s.sub && (
                              <span
                                className={cn(
                                  'text-[8px] opacity-40 mt-0.5 block',
                      ...
                      <motion.div
                        className="px-6 pt-7 pb-10 flex-shrink-0"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                      ...
                      >
                        <div className="relative">
                          ...
                          >
                            <span
                              className={cn(
                                ...
                                isDarkMode ? 'text-emerald-400/70' : 'text-emerald-600/80'
                              )}
                            >
                              IDEAL
                            </span>
                          ...
                          >
                            {/* Ideal zone — soft emerald lozenge */}
                            <motion.div
                              className="absolute top-0 bottom-0 rounded-full"
                              style={{
                                left: `${moIdealS}%`,
                                width: `${moIdealE - moIdealS}%`,
                                background:
                                  'linear-gradient(90deg, rgba(16,185,129,0.25) 0%, rgba(52,211,153,0.55) 50%, rgba(16,185,129,0.25) 100%)',
                                boxShadow:
                                  '0 0 14px rgba(52,211,153,0.35), inset 0 0 0 1px rgba(52,211,153,0.25)'
                              }}
                            ...
                            {/* Faint ideal-zone tick caps (no harsh white lines) */}
                            <span
                              className="absolute top-1/2 -translate-y-1/2 w-[1px] h-[10px] rounded-full"
                              style={{ left: `${moIdealS}%`, background: 'rgba(52,211,153,0.6)' }}
                            />
                            <span
                              className="absolute top-1/2 -translate-y-1/2 w-[1px] h-[10px] rounded-full"
                              style={{ left: `${moIdealE}%`, background: 'rgba(52,211,153,0.6)' }}
                            />
                          </div>

                          {/* Unified marker: thumb + tooltip share one horizontal anchor → caret always perfectly above dot */}
                          {!isLocked && (
                            <motion.div
                              className="absolute pointer-events-none z-10 flex flex-col items-center"
                              style={{ top: '-6px', transform: 'translateX(-50%)' }}
                              initial={{ left: '0%' }}
                              animate={{ left: `${moMarkerPct}%` }}
                              transition={{
                            ...
                            >
                              ...
                              >
                                ...
                                <div
                                  ...
                                  style={{
                                    background: `radial-gradient(circle at 30% 30%, rgba(255,255,255,0.95), ${scoreColor} 55%, ${scoreColor})`,
                                    boxShadow: `0 1px 2px rgba(0,0,0,0.4), 0 4px 14px ${scoreGlow}, inset 0 0 0 1px rgba(255,255,255,0.35)`
                                  }}
                                />
                              </motion.div>

                              {/* Tooltip — flex centers it directly under the thumb */}
                              <motion.div
                                className="mt-3"
                              ...
                              >
                                <div
                                  className="relative px-3 py-1.5 rounded-xl flex items-center justify-center whitespace-nowrap"
                                  style={{
                                    background: isDarkMode
                                      ? 'linear-gradient(180deg, rgba(24,24,27,0.95) 0%, rgba(9,9,11,0.95) 100%)'
                                      : 'linear-gradient(180deg, rgba(255,255,255,0.98) 0%, rgba(244,244,245,0.98) 100%)',
                                    boxShadow: isDarkMode
                                      ? `0 6px 20px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.06), 0 0 18px ${scoreGlow}`
                                      : `0 6px 18px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.06), 0 0 16px ${scoreGlow}`,
                                ...
                                >
                                  ...
                                  />
                                  <span
                                    className="text-[13px] font-semibold tabular-nums tracking-tight leading-none"
                                    style={{
                                      color: scoreColor,
                                      textShadow: `0 0 10px ${scoreGlow}`
                                    }}
                                  >
                                    {typeof mo.value === 'number' ? mo.value.toFixed(2) : mo.value}
                                    <span
                                      className={cn(
                                  ...
                                  {/* Upward caret (points to thumb above) */}
                                  <span
                                    aria-hidden
                                    ...
                                    style={{
                                      ...
                                      borderBottom: `5px solid ${isDarkMode ? 'rgba(24,24,27,0.95)' : 'rgba(255,255,255,0.98)'}`,
                                      filter: `drop-shadow(0 -1px 0 ${isDarkMode ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'})`
                                    }}
                                  />
                                </div>
                              </motion.div>
                          ...
                          <motion.div
                            className={cn(
                              'flex justify-between mt-16 text-[9px]',
                          ...
                          >
                            <span>0</span>
                            <span>
                              {mo.idealRange[0]}–{mo.idealRange[1]}
                              {mo.unit ?? ''}
                            </span>
                      ...
                      {!isLocked && moIndicators.length > 0 && (
                        <motion.div
                          className={cn(
                            'mx-5 mb-3 p-4 rounded-xl border',
                            isDarkMode
                              ...
                              >
                                <motion.div
                                  className={cn(
                                    ...
                                    isDarkMode ? 'bg-violet-400/60' : 'bg-violet-400'
                                  )}
                                  initial={{ scale: 0 }}
                                  animate={{ scale: 1 }}
                                  transition={{
                                ...
                                <span
                                  className={cn(
                                    ...
                                    isDarkMode ? 'text-white/60' : 'text-zinc-700'
                                  )}
                                >
                                  {tag}
                                </span>
                          ...
                          <div className="flex items-center gap-1.5 mb-2">
                            ...
                            >
                              <Zap
                                className={cn(
                                  'w-3.5 h-3.5',
                                  moWithin ? 'text-emerald-400' : 'text-amber-400'
                                )}
                              />
                            </motion.div>
                            <span
                            ...
                            >
                              {moWithin ? 'Optimal' : 'Improvement Tip'}
                            </span>
                          </div>
                          <p
                            className={cn(
