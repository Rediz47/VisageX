...
export function CelebritySection({
  ...
  return (
    ...
    >
      ...
      <h3
        className={cn(
          'text-lg md:text-xl font-display italic mb-4 md:mb-6 flex items-center',
      ...
      {celebrityResults.length > 0 ? (
          ...
          {celebrityResults[0] && celebrityResults[0].percentage >= 80 && (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-3xl font-display italic text-cyan-400">
                  {Number(celebrityResults[0].percentage || 0).toFixed(0)}%
                </span>
              </div>
              ...
              <div className="flex items-center justify-center gap-8">
                <div className="relative group">
                  ...
                  <img
                    ...
                    alt="You"
                    className={cn(
                      'relative w-24 h-24 rounded-2xl object-cover border-2',
                      isDarkMode ? 'border-white/10' : 'border-zinc-200'
                    )}
                  />
                  <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-black/80 text-white text-[9px] font-bold px-3 py-1 rounded-full">
                    YOU
                  </div>
                </div>
                ...
                <div className="relative group">
                  ...
                  <img
                    src={
                      celebrityResults[0].imageUrl ||
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(celebrityResults[0].name)}&background=random&color=fff&size=128`
                    }
              ...
              {celebrityResults[0].reason && (
                <div
                  className={cn(
                    'mt-6 p-4 rounded-xl border text-center text-sm italic',
                    isDarkMode
            ...
            {celebrityResults.map((celeb: any, i: number) => (
              ...
              >
                <div className="flex items-center gap-4 mb-3">
                  <div className="relative">
                    <img
                      src={
                        celeb.imageUrl ||
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(celeb.name)}&background=random&color=fff&size=128`
                      }
                      alt={celeb.name}
                      className={cn(
                        'w-16 h-16 rounded-xl object-cover border-2',
                        isDarkMode ? 'border-white/10' : 'border-zinc-200'
                      )}
                      referrerPolicy="no-referrer"
                      onError={(e) => {
                    ...
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Sparkles
                          ...
                          className={cn(
                            'w-3 h-3',
                            star <= Math.round(Number(celeb.percentage || 0) / 20)
                              ? 'text-amber-400'
                ...
                {celeb.reason && (
                  <p
                    className={cn(
                      'text-xs leading-relaxed opacity-80 italic',
        ...
        >
          ...
          <h4
            className={cn(
              'text-lg font-display italic mb-2',
              isDarkMode ? 'text-zinc-100' : 'text-zinc-900'
            )}
          >
          ...
          <p
            className={cn('text-sm mb-8 max-w-md', isDarkMode ? 'text-white/40' : 'text-zinc-500')}
          >
            Our AI can identify your exact celebrity lookalikes and models who share your facial
            structure.
          </p>
                ...
                {scanHistory.slice(-3).map((step, index, arr) => {
                  ...
                  return (
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                      className={cn(
                        'flex items-center gap-3 text-sm font-mono mb-2',
                        isCurrent
                          ? isDarkMode
                            ? 'text-cyan-400'
                            : 'text-cyan-600'
