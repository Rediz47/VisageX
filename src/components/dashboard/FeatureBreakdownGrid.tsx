...
const FeatureBreakdownGrid = React.memo(function FeatureBreakdownGrid({ data }: FeatureBreakdownGridProps) {
  ...
  const finalPoints = useMemo(() => {
    return features.map((f, i) => {
      ...
      const isItemLocked = isLocked && f.subject !== 'Symmetry' && f.subject !== 'Jawline' && f.subject !== 'Eyes';
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        isDarkMode ? "bg-[#0f0f13] border-white/5 shadow-2xl shadow-black/80" : "bg-white border-zinc-100 shadow-xl shadow-zinc-200/50"
      )}>
        ...
        <div className="flex items-start gap-4 mb-8">
          ...
          <div>
            <h3 className={cn("text-xl md:text-2xl font-bold tracking-tight mb-1", isDarkMode ? "text-cyan-400" : "text-cyan-600")}>
              Harmony Radar
            </h3>
            <p className={cn("text-xs md:text-sm", isDarkMode ? "text-cyan-100/50" : "text-cyan-600/70")}>
        ...
        <div className="relative w-full aspect-square max-w-[320px] mx-auto flex items-center justify-center mt-4">
          ...
          {isLocked && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center">
              <div className="bg-black/60 backdrop-blur-md px-6 py-4 rounded-3xl border border-white/10 flex flex-col items-center shadow-2xl">
                ...
                <p className="text-white/60 text-xs text-center mt-1">Reveal all 16 facial metrics</p>
              </div>
            </div>
          )}
          ...
          >
            ...
            {/* Labels */}
            {labels.map((lbl, i) => {
              const isLblLocked = isLocked && lbl.label !== 'Symmetry' && lbl.label !== 'Jawline' && lbl.label !== 'Eyes';
