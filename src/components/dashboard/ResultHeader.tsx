...
export function ResultHeader({ isDarkMode, onReset, onGenerateCard, isGeneratingCard }: ResultHeaderProps) {
  return (
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5 mb-8 md:mb-10">
      ...
      >
        <motion.button
          ...
          style={{
            ...
            color: isGeneratingCard
              ? (isDarkMode ? 'rgba(255,255,255,0.25)' : '#a1a1aa')
              : (isDarkMode ? '#e0e7ff' : '#4338ca'),
            border: `1px solid ${isGeneratingCard
        ...
        >
          ...
          <span className="relative z-10">{isGeneratingCard ? 'Generating...' : 'Export Card'}</span>
        </motion.button>
      </motion.div>
    </div>
  );
