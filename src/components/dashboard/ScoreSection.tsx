...
import { Lock, Share2, Check, Users, BarChart3, Trophy } from 'lucide-react';
...
function getScoreTier(score: number): { label: string; color: string; glow: string } {
  ...
  if (score >= 8.0)
    return { label: 'Outstanding', color: '#818cf8', glow: 'rgba(129,140,248,0.3)' };
  if (score >= 7.0)
    return { label: 'Above Average', color: '#818cf8', glow: 'rgba(129,140,248,0.25)' };
  if (score >= 5.5) return { label: 'Average', color: '#a1a1aa', glow: 'rgba(161,161,170,0.2)' };
  if (score >= 4.0)
    return { label: 'Below Average', color: '#f59e0b', glow: 'rgba(245,158,11,0.2)' };
...
/* ─────────────── Compact Harmony Radar (face card bottom panel) ─────────────── */
function CompactHarmonyRadar({
  data,
  isLocked
}: {
  ...
  const polar = (val: number, idx: number, total: number, radius: number) => {
    const angle = (Math.PI * 2 * idx) / total - Math.PI / 2;
    const r = (Math.max(0, Math.min(10, val)) / 10) * radius;
    return { x: CENTER + r * Math.cos(angle), y: CENTER + r * Math.sin(angle) };
  };

  const ringPolys = Array.from({ length: RINGS }, (_, i) => {
    const radius = MAX_R * ((i + 1) / RINGS);
    return Array.from({ length: n }, (_, j) => {
      const p = polar(10, j, n, radius);
      return `${p.x.toFixed(1)},${p.y.toFixed(1)}`;
    }).join(' ');
  });

  const axisEnds = Array.from({ length: n }, (_, i) => polar(10, i, n, MAX_R));

  const finalPts = data
    .map((d, i) => {
      const p = polar(d.A, i, n, MAX_R);
      return `${p.x.toFixed(1)},${p.y.toFixed(1)}`;
    })
    .join(' ');

  const initialPts = Array.from({ length: n }, () => `${CENTER},${CENTER}`).join(' ');
  ...
  return (
    ...
    >
      ...
      {/* Labels */}
      {data.map((d, i) => {
        const p = polar(10, i, n, MAX_R + 18);
        const anchor = Math.abs(p.x - CENTER) < 6 ? 'middle' : p.x > CENTER ? 'start' : 'end';
        return (
          <text
            key={`l${i}`}
            x={p.x}
      ...
      {/* Animated polygon */}
      <motion.polygon
        initial={{ points: initialPts }}
        animate={{ points: finalPts }}
        transition={{ duration: 1.1, delay: 0.4, ease: [0.16, 1, 0.3, 1] }}
        fill="url(#hr-fill)"
        stroke="#22d3ee"
        strokeWidth="1.5"
        filter="url(#hr-glow)"
        style={{ filter: isLocked ? 'blur(3px)' : undefined }}
      />
      ...
      {data.map((d, i) => {
        ...
        return (
          <motion.circle
            ...
            transition={{ duration: 1.1, delay: 0.4 + i * 0.04, ease: [0.16, 1, 0.3, 1] }}
            r={3}
            fill="#fff"
            stroke="#22d3ee"
            strokeWidth="1.4"
...
function BellCurveChart({
  ...
  const rightTail =
    rightTailPts
      .map(([x, y], i) => `${i === 0 ? 'M' : 'L'}${sx(x).toFixed(2)},${sy(y).toFixed(2)}`)
      .join('') +
    `L${sx(10).toFixed(2)},${sy(0).toFixed(2)} L${sx(score).toFixed(2)},${sy(0).toFixed(2)} Z`;
        ...
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: 'block' }}>
          <defs>
            ...
            <linearGradient id={`${uid.current}-fill`} x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="0%"
                stopColor={isDarkMode ? 'rgba(99,102,241,0.22)' : 'rgba(99,102,241,0.14)'}
              />
            ...
            <linearGradient id={`${uid.current}-stroke`} x1="0" y1="0" x2="1" y2="0">
              <stop
                offset="0%"
                stopColor={isDarkMode ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'}
              />
              <stop
                offset="45%"
                stopColor={isDarkMode ? 'rgba(129,140,248,0.5)' : 'rgba(99,102,241,0.4)'}
              />
          ...
          {!isLocked && (
            ...
            >
              ...
              <motion.circle
                ...
                strokeWidth="1.5"
                animate={{ scale: [0.78, 1.57], opacity: [0.5, 0] }}
              ...
              <circle cx={ux} cy={uy} r="5.5" fill={`url(#${uid.current}-pill)`} />
              <circle
                cx={ux}
                cy={uy}
                r="5.5"
...
export function ScoreSection({
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 md:gap-4 items-stretch">
      {/* ── Left: Face Photo (3D mouse-tilt on high-tier devices) ── */}
      <motion.div
        initial={{ opacity: 0, scale: 0.85, y: 30, rotate: -2 }}
        animate={{ opacity: 1, scale: 1, y: 0, rotate: 0 }}
        transition={{ duration: 1.0, ease: [0.16, 1, 0.3, 1], delay: 0.2 }}
        className="sm:w-[38%] lg:w-[34%] flex-shrink-0"
      >
        <TiltCard
          max={6}
          className={cn(
            'rounded-2xl md:rounded-3xl shadow-xl overflow-hidden relative group flex flex-col h-full',
        ...
        >
          ...
          {/* Bottom panel — compact Harmony Radar */}
          <div className="flex-1 flex flex-col justify-center items-center px-3 py-3 relative overflow-hidden">
            {/* Ambient glow */}
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                background: `radial-gradient(ellipse at 50% 80%, ${tier.glow}, transparent 70%)`,
                opacity: 0.2
              }}
            />

            <CompactHarmonyRadar data={radarData} isLocked={isLocked} />
            ...
            >
              ...
              <p className="text-[7px] font-mono text-white/15 mt-0.5 tracking-wider">
      ...
      <motion.div
        ...
        className={cn(
          'flex-1 rounded-2xl md:rounded-3xl shadow-xl relative overflow-hidden flex flex-col items-center justify-between py-5 px-4 md:px-6 text-center',
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(90deg, transparent, rgba(34,211,238,0.55), rgba(129,140,248,0.45), transparent)'
          }}
        />
        {/* Score-coloured ambient glow */}
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-48 pointer-events-none rounded-full blur-3xl"
          style={{ background: isLocked ? 'transparent' : tier.glow, opacity: 0.5 }}
        />

        <div className="relative z-10 flex flex-col items-center w-full gap-3.5">
          {/* Score display with arc gauge */}
          <div className="text-center relative">
            <p
              className={cn(
                'text-[10px] font-black uppercase tracking-[0.28em] mb-1',
                isDarkMode ? 'text-white/30' : 'text-zinc-900/30'
              )}
            >
            ...
            >
              ...
              <div className={cn('flex flex-col items-center justify-center', isLocked && 'blur-[14px] opacity-55 select-none')}>
                ...
                <motion.span
                  className={cn(
                    'text-[10px] font-bold uppercase tracking-[0.15em] mt-1',
            ...
            >
              <div
                className={cn(
                  'flex items-stretch gap-2 rounded-2xl p-2',
              ...
              >
                ...
                <div className="flex-1 text-center">
                  ...
                  <p
                    ...
                    style={{ color: '#818cf8' }}
                  >
                    {(structuralScore ?? overallScore).toFixed(1)}
                  </p>
                  <p
                    className={cn(
                      'text-[7px] mt-0.5',
                ...
                <div className="flex-1 text-center">
                  ...
                  {visualScore != null ? (
                    ...
                    <>
                      <p
                        className={cn(
                          'text-sm font-bold opacity-30 leading-none mt-1',
              ...
              {visualScore != null && (
                <p
                  className={cn(
                    'text-[8px] text-center mt-1.5 font-mono',
                    isDarkMode ? 'text-white/15' : 'text-zinc-400/60'
                  )}
                >
                  Overall = (Geometry + AI) / 2
                </p>
              )}
              ...
              {visualScore != null &&
                ...
                Math.abs(structuralScore - visualScore) >= 2.0 && (
                  <p
                    className={cn(
                      'text-[7px] text-center mt-1 italic',
                      isDarkMode ? 'text-amber-400/40' : 'text-amber-600/50'
                    )}
                  >
                    Differences may occur between structural measurements and visual analysis.
                  </p>
                )}
            </motion.div>
          </div>
          ...
          <div
            className={cn(
              'w-full rounded-2xl border p-2.5 md:p-3',
              isDarkMode ? 'bg-white/[0.025] border-white/[0.06]' : 'bg-zinc-50/70 border-zinc-200'
            )}
            style={{
              boxShadow: isDarkMode
                ? 'inset 0 1px 0 rgba(255,255,255,0.04), 0 18px 45px rgba(0,0,0,0.22)'
                : '0 18px 45px rgba(99,102,241,0.08)'
            }}
          >
            <div className="flex items-center justify-between px-1 mb-2">
              <div className="text-left">
                ...
                <p className={cn('text-[11px]', isDarkMode ? 'text-white/30' : 'text-zinc-500')}>
                  Compared with the VisageX scan population
                </p>
              </div>
              <span
                className="px-2.5 py-1 rounded-full text-[10px] font-black tabular-nums"
                style={{
                  color: tier.color,
                  background: isDarkMode ? 'rgba(99,102,241,0.12)' : 'rgba(99,102,241,0.08)',
                  border: `1px solid ${isDarkMode ? 'rgba(99,102,241,0.24)' : 'rgba(99,102,241,0.16)'}`
                }}
              >
                Top {topPercentile}%
              </span>
          ...
          {isLocked ? (
            <motion.button
              onClick={onOpenPricing || scrollToPricing}
            ...
            >
              ...
              <div
                className="relative flex items-center gap-2.5 px-6 py-2.5 rounded-full border overflow-hidden"
                style={{
            ...
            <div className="flex items-center gap-2.5">
              ...
              >
                ...
                <span className="text-[10px] font-black text-white uppercase tracking-[0.12em] relative z-10">
                  Share
                </span>
              </motion.button>
            </div>
          ...
          >
            {[
              ...
              <React.Fragment key={stat.label}>
                {idx > 0 && (
                  <div
                    className={cn('w-px h-6', isDarkMode ? 'bg-white/[0.05]' : 'bg-zinc-200/50')}
                  />
                )}
                <div className="flex-1 flex items-center justify-center gap-1.5">
                  <stat.icon
                    className={cn(
                      'w-3 h-3',
                      isDarkMode ? 'text-indigo-400/40' : 'text-indigo-500/30'
                    )}
                  />
                  <span
                    className={cn(
