...
import { ChevronDown, Info, Shield, TrendingUp, BookOpen } from 'lucide-react';
 ...
 */
export function FacialRatioCard({ ratio, isDarkMode, isLocked = false, index = 0 }: FacialRatioCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  const numValue = typeof ratio.value === 'number' ? ratio.value : parseFloat(ratio.value as string);
  ...
  const getScoreColor = (s: number) => {
    if (s >= 8) return { text: 'text-emerald-400', bg: 'bg-emerald-400', border: 'border-emerald-500/20' };
    if (s >= 6) return { text: 'text-amber-400', bg: 'bg-amber-400', border: 'border-amber-500/20' };
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isDarkMode ? 'bg-black border-white/5 hover:border-white/10' : 'bg-white border-zinc-200 hover:border-zinc-300',
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="text-right">
            <span className={cn('text-xl md:text-2xl font-display italic', isDarkMode ? 'text-zinc-100' : 'text-zinc-900')}>
              {isLocked ? '??' : typeof ratio.value === 'number' ? ratio.value.toFixed(2) : ratio.value}
            </span>
            {ratio.unit && (
              <span className={cn('text-[10px] ml-0.5 opacity-40', isDarkMode ? 'text-white' : 'text-zinc-900')}>
                {ratio.unit}
              </span>
            )}
          </div>
          ...
          >
            <div className={cn(
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className={cn('p-3 rounded-xl text-center', isDarkMode ? 'bg-white/5' : 'bg-zinc-50')}>
                  <p className="text-[8px] font-bold uppercase tracking-widest opacity-40 mb-1">Measured</p>
                  <p className={cn('text-lg font-display italic font-bold', isDarkMode ? 'text-cyan-400' : 'text-cyan-600')}>
                    {typeof ratio.value === 'number' ? ratio.value.toFixed(2) : ratio.value}{ratio.unit || 'x'}
                  </p>
                </div>
                <div className={cn('p-3 rounded-xl text-center', isDarkMode ? 'bg-white/5' : 'bg-zinc-50')}>
                  <p className="text-[8px] font-bold uppercase tracking-widest opacity-40 mb-1">Ideal Range</p>
                  <p className={cn('text-lg font-display italic font-bold', isDarkMode ? 'text-emerald-400' : 'text-emerald-600')}>
                    {ratio.idealRange[0]}–{ratio.idealRange[1]}{ratio.unit || 'x'}
                  </p>
                </div>
                <div className={cn('p-3 rounded-xl text-center', isDarkMode ? 'bg-white/5' : 'bg-zinc-50')}>
                  <p className="text-[8px] font-bold uppercase tracking-widest opacity-40 mb-1">Score</p>
                  <p className={cn('text-lg font-display italic font-bold', scoreColors.text)}>
                    {ratio.score.toFixed(1)}<span className="text-xs opacity-30">/10</span>
                  </p>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className={cn('text-[9px] font-bold uppercase tracking-widest opacity-40', isDarkMode ? 'text-white' : 'text-zinc-900')}>
                    Distribution Scale
                  </span>
                </div>
                <div className="relative h-6 rounded-full overflow-hidden">
                  ...
                  >
                    <div
                      ...
                      style={{
                        backgroundColor: isWithinIdeal ? 'rgba(52,211,153,0.8)' : 'rgba(244,63,94,0.8)',
                  ...
                  {/* Range labels */}
                  <span className="absolute bottom-0.5 left-1 text-[7px] font-mono opacity-30 text-white">{ratio.minRange}</span>
                  <span className="absolute bottom-0.5 right-1 text-[7px] font-mono opacity-30 text-white">{ratio.maxRange}</span>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className={cn('text-[9px] font-bold uppercase tracking-widest opacity-40 flex items-center gap-1', isDarkMode ? 'text-white' : 'text-zinc-900')}>
                    <Shield className="w-3 h-3" />
                    Confidence
                  </span>
                  <span className={cn('text-[10px] font-bold', ratio.confidence >= 80 ? 'text-emerald-400' : ratio.confidence >= 50 ? 'text-amber-400' : 'text-rose-400')}>
                    {ratio.confidence}%
                  </span>
                </div>
                <div className={cn('w-full h-1.5 rounded-full overflow-hidden', isDarkMode ? 'bg-white/5' : 'bg-zinc-100')}>
                  <div
                    className="h-full rounded-full confidence-bar"
                    style={{
                      '--confidence-width': `${ratio.confidence}%`,
                      backgroundColor: ratio.confidence >= 80 ? '#34d399' : ratio.confidence >= 50 ? '#fbbf24' : '#f43f5e',
                      width: 0,
              ...
              <div className={cn(
                ...
                <p className={cn('text-xs leading-relaxed', isDarkMode ? 'text-white/60' : 'text-zinc-600')}>
                  {ratio.description}
                </p>
              </div>
            </div>
 ...
 */
export function generateRatioData(metrics: Record<string, string | number>, breakdown: Record<string, number>): RatioData[] {
  ...
  if (metrics.fWHR) {
    ...
    if (!isNaN(val)) {
      ratios.push({
        ...
        maxRange: 2.6,
        unit: 'x',
        description: 'The fWHR measures the width of the face at the cheekbones divided by the upper-face height. Research links this ratio to perceived dominance and attractiveness. Values near 1.9-2.0 are considered most harmonious.',
  ...
  if (metrics.goldenRatio) {
    const val = typeof metrics.goldenRatio === 'number' ? metrics.goldenRatio : parseFloat(String(metrics.goldenRatio));
    if (!isNaN(val)) {
      ratios.push({
        ...
        maxRange: 2.0,
        unit: 'φ',
        description: 'The Golden Ratio (1.618) appears throughout nature and classical art. This measures how closely your facial proportions align with this ideal. Higher adherence suggests greater perceived harmony.',
  ...
  if (metrics.facialSymmetry) {
    const val = typeof metrics.facialSymmetry === 'number' ? metrics.facialSymmetry : parseFloat(String(metrics.facialSymmetry).replace('%', ''));
    if (!isNaN(val)) {
      ...
      ratios.push({
        ...
        maxRange: 100,
        unit: '%',
        description: 'Bilateral symmetry measures how closely the left and right halves of the face mirror each other across 468 landmark points. Higher values indicate superior genetic expression and developmental stability.',
  ...
  if (metrics.canthalTilt) {
    const val = typeof metrics.canthalTilt === 'number' ? metrics.canthalTilt : parseFloat(String(metrics.canthalTilt).replace('°', ''));
    if (!isNaN(val)) {
      ratios.push({
        ...
        maxRange: 15,
        unit: '°',
        description: 'The canthal tilt measures the angle between the inner and outer corners of the eye. A positive tilt (4-8°) is associated with a youthful, alert appearance. Negative tilts can create a tired look.',
  ...
  if (breakdown.Eyes !== undefined) {
    ratios.push({
      ...
      maxRange: 4.0,
      unit: 'x',
      description: 'The ratio of eye width to height (palpebral fissure) indicates eye shape. Almond-shaped eyes with ratios near 2.8-3.0x are considered most aesthetically pleasing across cultures.',
  ...
  if (breakdown.Jawline !== undefined) {
    ratios.push({
      ...
      maxRange: 160,
      unit: '°',
      description: 'The gonial angle measures the sharpness of the jaw angle. Lower angles (120-130°) indicate a more defined, angular jawline associated with perceived masculinity and attractiveness.',
