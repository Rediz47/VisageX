...
export function Auth({ isOpen, onClose, isDarkMode, initialMode = 'signin', initialReferralCode = '' }: AuthProps) {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        await setDoc(doc(db, 'users', user.uid), {
          ...
          createdAt: serverTimestamp(), role: 'user', credits: DEV_EMAILS.includes(user.email ?? '') ? 9999 : 0,
          referralCode: myReferralCode, invitedCount: 0
        });
        try {
      ...
      } else if (DEV_EMAILS.includes(user.email ?? '')) {
        const idToken = await user.getIdToken();
        await fetch('/api/dev/boost-credits', { method: 'POST', headers: { Authorization: `Bearer ${idToken}` } });
      }
      onClose();
    ...
    try {
      ...
      if (isSignUp) {
        ...
        await setDoc(doc(db, 'users', result.user.uid), {
          uid: result.user.uid, email: result.user.email, displayName: name, referredBy: null,
          createdAt: serverTimestamp(), role: 'user', credits: ['choxeliluka43@gmail.com','lukachoxeli918@gmail.com'].includes(result.user.email ?? '') ? 9999 : 0,
          referralCode: myReferralCode, invitedCount: 0
        });
        ...
        if (referralInput) {
          try {
            await fetch('/api/referral/redeem', {
              ...
              body: JSON.stringify({ userId: result.user.uid, referralCode: referralInput, fingerprint })
            });
          } catch (e) { console.error('Failed to redeem referral code', e); }
        }
        try {
          await fetch('/api/email/welcome', {
            method: 'POST',
        ...
        const DEV_EMAILS = ['choxeliluka43@gmail.com', 'lukachoxeli918@gmail.com'];
        if (DEV_EMAILS.includes(signInResult.user.email ?? '')) {
  ...
  const iconClass = `absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${
    isDarkMode ? 'text-white/25 group-focus-within:text-indigo-400' : 'text-zinc-400 group-focus-within:text-indigo-500'
  }`;
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
        ...
        <motion.div
          ...
          className={`relative w-full max-w-md overflow-hidden rounded-[2.5rem] border shadow-2xl ${
            isDarkMode ? 'bg-[#050508] border-white/[0.08]' : 'bg-white border-indigo-100 shadow-indigo-500/10'
          }`}
        >
          <button
            onClick={onClose}
          ...
          <div className="p-8 md:p-12">
            <div className="text-center mb-10">
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-6"
                style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.15), rgba(34,211,238,0.1))', border: '1px solid rgba(99,102,241,0.2)' }}>
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div className="relative group">
                  ...
                  <input type="email" placeholder="Email Address" required value={resetEmail}
                    onChange={e => setResetEmail(e.target.value)} className={inputClass} />
                </div>
                {error && (
                ...
                <button type="submit" disabled={loading} className="btn-primary w-full justify-center !rounded-2xl">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
      ...
      <motion.div
        ...
        style={{ boxShadow: isDarkMode ? '0 40px 120px rgba(0,0,0,0.8), 0 0 0 1px rgba(99,102,241,0.1)' : '0 40px 120px rgba(99,102,241,0.15), 0 0 0 1px rgba(99,102,241,0.12)' }}
      >
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-0 w-72 h-72 rounded-full" style={{ background: 'radial-gradient(ellipse, rgba(99,102,241,0.25) 0%, transparent 70%)', filter: 'blur(30px)' }} />
            <div className="absolute bottom-0 right-0 w-56 h-56 rounded-full" style={{ background: 'radial-gradient(ellipse, rgba(34,211,238,0.15) 0%, transparent 70%)', filter: 'blur(30px)' }} />
          </div>

          <div className="relative z-10">
            {/* Logo */}
            <div className="flex items-center gap-2.5 mb-12">
              <div className="w-9 h-9"><img src="/logo.png" alt="VisageX" className="w-full h-full object-contain" /></div>
              <span className="font-display font-extrabold text-xl tracking-tighter text-white">
                VISAGE<span style={{ background: 'linear-gradient(135deg, #6366f1, #22d3ee)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>X</span>
              </span>
            </div>

            <h2 className="text-3xl font-display font-black text-white leading-tight mb-4 italic">
              Discover your<br />
              <span style={{ background: 'linear-gradient(135deg, #a78bfa 0%, #22d3ee 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                true potential.
              </span>
            </h2>
        ...
        <div className={`flex-1 flex flex-col p-8 md:p-12 ${isDarkMode ? '' : ''}`}>
          ...
          <div className="mb-8">
            ...
            <div className="flex md:hidden items-center gap-2 mb-6">
              <div className="w-8 h-8"><img src="/logo.png" alt="VisageX" className="w-full h-full object-contain" /></div>
            </div>
            <h2 className={`text-2xl font-display font-extrabold tracking-tight mb-1 ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>
              {isSignUp ? 'Create Account' : 'Welcome Back'}
            </h2>
            <p className={`text-sm ${isDarkMode ? 'text-white/40' : 'text-zinc-500'}`}>
              {isSignUp ? 'Join VisageX for personalized neural insights.' : 'Sign in to access your facial analysis history.'}
            </p>
          </div>
          ...
          <div className="relative mb-6">
            <div className={`absolute inset-0 flex items-center`}>
              <div className={`w-full border-t ${isDarkMode ? 'border-white/[0.06]' : 'border-zinc-200'}`} />
            </div>
            <div className="relative flex justify-center text-[10px] uppercase tracking-[0.3em] font-black">
              <span className={`px-4 ${isDarkMode ? 'bg-[#050508] text-white/25' : 'bg-white text-zinc-400'}`}>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-3.5 flex-1">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className={`text-[10px] font-black uppercase tracking-widest transition-colors ${
                    isDarkMode ? 'text-white/25 hover:text-indigo-400' : 'text-zinc-400 hover:text-indigo-500'
                  }`}
                >
                  Forgot password?
                </button>
            ...
            {isSignUp && (
              <div className="relative group">
                <div className={iconClass}><ArrowRight className="w-4 h-4" /></div>
                <input type="text" name="referralCode" placeholder="Referral Code (Optional)"
                  defaultValue={initialReferralCode} className={inputClass} />
              </div>
            )}
            ...
            <button type="submit" disabled={loading || (isSignUp && !turnstileToken)}
          ...
          <div className="mt-6 text-center">
            <button
              ...
              className={`text-xs font-black uppercase tracking-widest transition-colors ${
                isDarkMode ? 'text-white/30 hover:text-indigo-400' : 'text-zinc-400 hover:text-indigo-500'
              }`}
            >
