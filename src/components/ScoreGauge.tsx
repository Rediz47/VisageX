...
export function ScoreGauge({
  ...
  // One-shot guard: the arc + counter must NEVER re-run on re-render.
  ...
  // Animated counter (one-shot, tier-aware, clamped to 0.8s).
  ...
  useEffect(() => {
    ...
    // Low tier or reduced-motion: snap to final value, no tween.
    if (!preset.flags.enableCounterTween || prefersReducedMotion) {
      el.textContent = score.toFixed(1);
      counterDoneRef.current = true;
      return;
    }

    if (!isVisible || counterDoneRef.current) return;

    const clamped = Math.max(0.4, Math.min(preset.durations.slow + 0.1, 0.8));
    ...
    score,
    delay,
    preset.flags.enableCounterTween,
    preset.durations.slow,
