...
interface Plan {
  ...
  savings?: string; popular?: boolean; description: string; features: string[];
}
const PLANS: Plan[] = [
  {
    id: 'price_single', name: 'Trial', price: '$1.49', credits: 1,
  ...
  {
    id: 'price_basic', name: 'Explorer', price: '$4.99', credits: 5,
    popular: true, savings: 'Save 30%',
    description: 'Track improvement and build your roadmap.',
    features: ['5 Full Face Analyses', 'Step-by-step improvement path', 'Progress tracking across scans', 'Priority processing'],
  },
  {
    id: 'price_pro', name: 'Serious', price: '$12.99', credits: 15,
    savings: 'Save 40%',
    description: 'Deep optimization across 15 detailed reports.',
    features: ['15 Full Face Analyses', 'High-res strength/weakness map', 'Priority processing', 'Full progress history'],
  },
];

interface PricingProps {
  isDarkMode: boolean; userId: string; userEmail: string;
  onClose: () => void; overallScore?: number;
}
      ...
      <div className={cn('fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 backdrop-blur-2xl', overlay)}>
        <AnimatePresence mode="wait">
          ...
          {loading === 'success' ? (
            <motion.div key="su" initial={{ opacity:0, scale:0.9, y:20 }} animate={{ opacity:1, scale:1, y:0 }}
              ...
              <motion.h2 initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25 }}
                className={cn('text-4xl font-black mb-4 tracking-tight', text)}>You're all set.</motion.h2>
              <motion.p initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.35 }}
                className={cn('text-base mb-10', textSub)}>Credits added. Your full analysis is now unlocked.</motion.p>
              <motion.button initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.45 }}
                whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }} onClick={onClose}
          ...
          /* ─── VERIFYING ───────────────────────────────────────────── */
            ...
            <motion.div key="ca" initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
              className={cn('w-full max-w-sm p-14 rounded-[2.5rem] border shadow-2xl text-center', bg, border)}>
              <div className="relative w-16 h-16 mx-auto mb-8">
                <div className={cn('w-16 h-16 rounded-full border-4 animate-spin', isDarkMode ? 'border-white/5 border-t-white' : 'border-zinc-200 border-t-zinc-700')} />
            ...
            <motion.div key="checkout"
              ...
              <div className="absolute top-6 left-6 z-20">
                <button onClick={() => setSelectedPlan(null)}
                  className={cn('flex items-center gap-1.5 px-4 py-2.5 rounded-full border text-xs font-black uppercase tracking-widest transition-all', borderSub, textSub, hoverBg)}>
                  <ChevronLeft className="w-4 h-4" /> Back
                </button>
              </div>
              <button onClick={onClose}
                className={cn('absolute top-6 right-6 z-20 p-2.5 rounded-full border transition-all', borderSub, textSub, hoverBg)}>
                <X className="w-5 h-5" />
              </button>

              {/* LEFT — Payment */}
              <div className={cn('flex-[1.1] p-8 pt-20 md:p-14 md:pt-24 flex flex-col gap-8', isDarkMode ? 'border-r border-white/5' : 'border-r border-zinc-100')}>
                <div>
                  <p className={cn('text-[10px] uppercase font-black tracking-widest mb-3', textMuted)}>Secure Checkout</p>
                  <h2 className={cn('text-3xl font-black tracking-tight', text)}>Configure your plan</h2>
                </div>

                <div>
                  <p className={cn('text-[10px] uppercase font-black tracking-widest mb-4', textMuted)}>Payment method</p>
                  <div className={cn('p-5 rounded-2xl border flex items-center justify-between mb-5', inputBg)}>
                    <div className="flex items-center gap-3">
                      <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center', isDarkMode ? 'bg-indigo-500/10' : 'bg-indigo-50')}>
                        <CreditCard className="w-4 h-4 text-indigo-500" />
                      </div>
                      <span className={cn('text-sm font-bold', text)}>PayPal · Credit &amp; Debit Card</span>
                    </div>
                    <div className="flex items-center gap-2 opacity-30">
                      <svg viewBox="0 0 50 16" className="h-4 w-auto" fill={isDarkMode ? 'white' : '#111'}>
                        <text y="13" fontSize="14" fontWeight="bold" letterSpacing="1">VISA</text>
                      </svg>
                  ...
                  <PayPalBtn planId={selectedPlan.id} createOrder={createOrder} onApprove={onApprove} />
                </div>

                <div className={cn('flex items-center justify-center gap-6 text-[9px] uppercase font-black tracking-widest', textMuted, 'opacity-60')}>
                  <span className="flex items-center gap-1.5"><Lock className="w-3 h-3" /> Secure</span>
                  <span className="flex items-center gap-1.5"><Shield className="w-3 h-3" /> SSL</span>
                ...
                <p className={cn('text-[9px] leading-relaxed', textMuted)}>
                  By completing this purchase you agree to our Terms of Service. Credits are added instantly to your account.
                </p>
              </div>
              ...
              <div className={cn('flex-1 flex flex-col p-8 pt-20 md:p-14 md:pt-24 gap-8', bgSub)}>
                  <div className="mb-8">
                    <p className={cn('text-xs uppercase font-black tracking-[0.2em] mb-3', textMuted)}>Your Plan</p>
                    <h3 className={cn('text-2xl md:text-3xl font-black mb-1.5', text)}>{selectedPlan.name}</h3>
                    <p className={cn('text-sm', textSub)}>{selectedPlan.description}</p>
                  </div>

                <div>
                  <p className={cn('text-[10px] uppercase font-black tracking-widest mb-4', textMuted)}>What's included</p>
                ...
                <div className={cn('mt-auto border-t pt-8 space-y-3', borderSub)}>
                  <div className={cn('flex justify-between text-sm', textSub)}>
                    <span>{selectedPlan.name} ({selectedPlan.credits} credit{selectedPlan.credits > 1 ? 's' : ''})</span>
                    <span>{selectedPlan.price}</span>
                  </div>
                  {selectedPlan.savings && (
                    <div className="flex justify-between text-sm text-emerald-500 font-bold">
                      <span>Discount</span><span>{selectedPlan.savings}</span>
                    </div>
                  )}
                  <div className={cn('flex justify-between text-sm', textSub)}>
                  ...
                  <div className={cn('flex justify-between font-black text-xl pt-2 border-t', text, borderSub)}>
                    <span>Due today</span><span>{selectedPlan.price}</span>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          /* ─── PLAN SELECTION ────────────────────────────────────────── */
            ...
            <motion.div key="selection"
              ...
              <div className={cn('relative px-8 md:px-12 pt-8 pb-7 border-b', borderSub)}>
                <button onClick={onClose}
                  className={cn('absolute top-8 right-8 p-2.5 rounded-full border transition-all', borderSub, textSub, hoverBg)}>
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="p-8 md:p-14">
                  ...
                  {PLANS.map((plan, i) => (
                    <motion.div key={plan.id}
                      ...
                      <div className="mb-4 mt-1">
                        <p className={cn('text-[10px] font-black uppercase tracking-[0.25em] mb-3', textMuted)}>
                          {plan.name}
                        </p>
                        <div className="flex items-baseline gap-2 mb-2">
                          <span className={cn('text-[2.5rem] font-black tracking-tighter leading-none', text)}>
                            {plan.price}
                          </span>
                        ...
                        {plan.savings && (
                          <span className="inline-block text-xs font-black text-emerald-500">{plan.savings}</span>
                        )}
                        ...
                        {plan.features.map((f, fi) => (
                          <li key={fi} className="flex items-center gap-3 text-sm">
                            <div className={cn('w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0',
                              ...
                              <Zap className={cn('w-3 h-3', plan.popular ? 'text-indigo-500' : textMuted)} />
                            </div>
                ...
                {/* Trust footer */}
                <div className={cn('mt-6 pt-6 border-t flex flex-col md:flex-row items-center justify-center gap-6 text-[10px] font-black uppercase tracking-[.2em]', borderSub, textMuted)}>
                  <span className="flex items-center gap-2.5"><Shield className="w-4 h-4 text-indigo-400" /> 256-bit SSL Encrypted</span>
                  <span className="hidden md:block opacity-30">·</span>
                  <span className="flex items-center gap-2.5"><CreditCard className="w-4 h-4 text-indigo-400" /> All Major Cards Accepted</span>
                  <span className="hidden md:block opacity-30">·</span>
                  <span className="flex items-center gap-2.5"><Zap className="w-4 h-4 text-indigo-400" /> Instant Credit Delivery</span>
                </div>
              </div>
            </motion.div>
          )}
