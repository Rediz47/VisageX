...
const referralLimiter = createSharedRateLimiter(
  5,
  '15 m',
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (suspicious) {
        console.warn(
          `[Fraud] Multi-account abuse suspected for referral: user=${userId} ip=${clientIp}`
        );
        return res
          .status(403)
          .json({ error: 'Suspicious activity detected. Please contact support.' });
      }
      ...
      return res
        .status(200)
        .json({ success: true, message: 'Referral applied! You received +1 bonus credit.' });
    } catch (error: any) {
...
router.get('/leaderboard', async (req, res) => {
  try {
    ...
    try {
      ...
      /* Redis miss — fall through to Firestore */
    }

    if (!entries) {
      const snap = await db.collection('users').orderBy('invitedCount', 'desc').limit(10).get();
      ...
      if (redis && rawEntries) {
        ...
        try {
          await redis.set('leaderboard:top5', JSON.stringify(cacheSafe), { ex: 60 });
        } catch {
          /* ignore */
        }
      }
