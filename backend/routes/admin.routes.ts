...
const ADMIN_EMAILS = getAdminEmails().length > 0
  ? getAdminEmails()
  : ['choxeliluka43@gmail.com', 'lukachoxeli918@gmail.com'];
...
router.get('/analytics', requireAuth, requireAdmin, async (_req, res) => {
  try {
    ...
    const recentOrdersSnap = await db.collection('processed_orders')
      .where('processedAt', '>=', weekAgo.toISOString())
      .count().get();
    const ordersLast7d = recentOrdersSnap.data().count;

    // 7. Total credits in circulation
    const top50 = await db.collection('users')
      .orderBy('credits', 'desc')
      .limit(50)
      .get();
    let totalCreditsInCirculation = 0;
    top50.forEach(doc => { totalCreditsInCirculation += doc.data().credits || 0; });
