...
// IMPORTANT: Top-level imports are kept minimal so that the Lambda init phase
// stays fast. All heavy modules (firebase-admin, sharp, helmet, pino,
// http-proxy-middleware, route handlers) are dynamically imported inside
// `configureApp()` which runs on the first request, where the much larger
// per-invocation timeout applies. This avoids `Runtime.ExitError exit 129`
// (SIGHUP) during cold starts caused by exceeding Lambda's init budget.

export const app = express();

let configured = false;
let configurePromise: Promise<typeof app> | null = null;

export function configureApp(): Promise<typeof app> {
  if (configured) return Promise.resolve(app);
  ...
  // otherwise flood stdout and make the terminal unreadable).
    ...
    app.use('/api/scans', scanRoutes.default);
