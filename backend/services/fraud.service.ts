...
async function raiseSignal(
  ...
  try {
    ...
    console.warn(
      `[Fraud] Signal raised: ${type} for user ${userId} (severity: ${severity}, action: ${action})`
    );
  } catch (err) {
...
export async function isHighRiskForExpensiveOp(userId: string): Promise<{
  ...
  try {
    ...
    if ((profile.riskScore || 0) >= RISK_SCORE_BLOCK_EXPENSIVE) {
      console.warn(
        `[Fraud] Preemptive block: user ${userId} has riskScore ${profile.riskScore} (threshold: ${RISK_SCORE_BLOCK_EXPENSIVE})`
      );
      return {
        blocked: true,
