...
export function analyzeSymmetry(l: LandmarkArray): SymmetryResult {
  ...
  const detailedSymmetry = symmetryPairs.map((pair) => {
    ...
    if (horizontalSymmetry < 0.95) {
      observation =
        leftDistX > rightDistX
          ? 'Left side appears slightly wider'
          : 'Right side appears slightly wider';
    } else if (verticalDiff > 0.02) {
      observation =
        pair.left.y < pair.right.y
          ? 'Left side is slightly elevated'
          : 'Right side is slightly elevated';
    }
  ...
  const avgSymmetryScore =
    detailedSymmetry.reduce((acc, curr) => acc + curr.score, 0) / detailedSymmetry.length;
  return { symmetryScore: avgSymmetryScore / 10, detailedSymmetry, avgSymmetryScore };
}

const clamp01 = (value: number) => Math.max(0, Math.min(1, value));

const rangeScore = (value: number, idealMin: number, idealMax: number, tolerance: number) => {
...
const angleAt = (a: Landmark, b: Landmark, c: Landmark) => {
  ...
  return Math.acos(clamp01(dot / (magA * magC)) * 2 - 1) * (180 / Math.PI);
};

export function calculateFaceShape(l: LandmarkArray): { shape: string; confidence: number } {
  const faceHeight = dist(l[10], l[152]);
  const cheekWidth = dist(l[234], l[454]);
  ...
  const heightToCheek = faceHeight / cheekWidth;
  ...
  const cheekDominance = cheekWidth / Math.max(foreheadWidth, jawWidth);
  const chinAngle = angleAt(l[132], l[152], l[361]);
  const sharpChin = rangeScore(chinAngle, 42, 72, 38);
  const softChin = rangeScore(chinAngle, 82, 125, 42);
  const squareChin = rangeScore(chinAngle, 68, 95, 35);
  ...
  scores.oblong += rangeScore(heightToCheek, 1.62, 1.95, 0.22) * 3.4;
  scores.oblong += rangeScore(jawToCheek, 0.72, 0.9, 0.22) * 0.8;
  scores.oblong += rangeScore(foreheadToCheek, 0.78, 0.98, 0.25) * 0.7;

  scores.round += rangeScore(heightToCheek, 1.18, 1.42, 0.22) * 2.2;
  scores.round += rangeScore(jawToCheek, 0.82, 0.98, 0.18) * 1.2;
  scores.round += softChin * 1.4;

  scores.square += rangeScore(heightToCheek, 1.28, 1.58, 0.25) * 1.4;
  scores.square += rangeScore(jawToCheek, 0.9, 1.05, 0.18) * 2.1;
  scores.square += rangeScore(foreheadToCheek, 0.85, 1.05, 0.2) * 1;
  scores.square += squareChin * 1.2;

  scores.heart += rangeScore(foreheadToCheek, 0.95, 1.18, 0.2) * 1.8;
  scores.heart += rangeScore(jawToCheek, 0.58, 0.82, 0.18) * 1.9;
  scores.heart += rangeScore(heightToCheek, 1.28, 1.62, 0.3) * 0.8;
  scores.heart += sharpChin * 1.1;

  scores.diamond += rangeScore(cheekDominance, 1.08, 1.28, 0.18) * 2.4;
  scores.diamond += rangeScore(jawToCheek, 0.58, 0.84, 0.18) * 1.4;
  scores.diamond += rangeScore(foreheadToCheek, 0.68, 0.9, 0.18) * 1.2;
  scores.diamond += sharpChin * 0.8;

  scores.triangle += rangeScore(jawToForehead, 1.04, 1.32, 0.2) * 2.2;
  scores.triangle += rangeScore(jawToCheek, 0.86, 1.08, 0.18) * 1.4;
  scores.triangle += rangeScore(foreheadToCheek, 0.65, 0.9, 0.2) * 1;

  scores.oval += rangeScore(heightToCheek, 1.38, 1.65, 0.22) * 1.8;
  scores.oval += rangeScore(jawToCheek, 0.72, 0.9, 0.2) * 1.2;
  scores.oval += rangeScore(foreheadToCheek, 0.78, 1.0, 0.2) * 1.1;
  scores.oval += rangeScore(cheekDominance, 1.0, 1.12, 0.14) * 0.9;

  const ranked = Object.entries(scores).sort((a, b) => b[1] - a[1]);
  const [shape, topScore] = ranked[0];
  const secondScore = ranked[1]?.[1] ?? 0;
  const confidence = clamp01(0.45 + ((topScore - secondScore) / Math.max(topScore, 1)) * 0.55);
...
export function calculateMetrics(
  ...
  const leftTilt =
    Math.atan2(leftEyeInner.y - leftEyeOuter.y, leftEyeInner.x - leftEyeOuter.x) * (180 / Math.PI);
  const rightTilt =
    Math.atan2(rightEyeInner.y - rightEyeOuter.y, rightEyeOuter.x - rightEyeInner.x) *
    (180 / Math.PI);
  ...
  const faceShapeResult = calculateFaceShape(l);
  ...
  const overallScore =
    (symmetryScore + finalEyeScore + finalJawScore + proportionsScore + fwhrScore) / 5;
  ...
  // ── Canthal tilt ──────────────────────────────────────────────────
  if (avgCanthalTilt > 5 && avgCanthalTilt < 12)
    strengths.push(`Sharp positive canthal tilt (${avgCanthalTilt.toFixed(1)}°)`);
  else if (avgCanthalTilt > 2)
    strengths.push(`Positive canthal tilt (${avgCanthalTilt.toFixed(1)}°)`);
  else if (avgCanthalTilt <= 0)
    weaknesses.push(`Negative or neutral canthal tilt (${avgCanthalTilt.toFixed(1)}°)`);
  else if (avgCanthalTilt <= 2)
    weaknesses.push(`Below-average canthal tilt (${avgCanthalTilt.toFixed(1)}°)`);

  // ── fWHR ──────────────────────────────────────────────────────────
  if (fWHR >= 1.75 && fWHR <= 1.95)
    strengths.push(`Ideal facial width-to-height ratio (${fWHR.toFixed(2)})`);
  ...
  // ── Jawline ───────────────────────────────────────────────────────
  if (cheekToJawRatio >= 1.2 && cheekToJawRatio <= 1.35)
    strengths.push('Strong, well-proportioned jawline');
    ...
    weaknesses.push('Overly square jawline — unfavorable cheek-to-jaw ratio');

  // ── Golden Ratio / Proportions ────────────────────────────────────
  if (Math.abs(1.618 - heightToWidthRatio) < 0.08)
    strengths.push('Excellent Golden Ratio proportions');
  else if (Math.abs(1.618 - heightToWidthRatio) > 0.25)
    weaknesses.push('Face shape deviates significantly from the Golden Ratio');
  ...
  // ── Lower face ────────────────────────────────────────────────────
  if (lowerFaceRatio > 0.38)
    weaknesses.push('Long lower face — disproportionate chin-to-nose distance');
  else if (lowerFaceRatio < 0.28) weaknesses.push('Short lower face — compressed chin projection');
  else if (lowerFaceRatio >= 0.31 && lowerFaceRatio <= 0.36)
    strengths.push('Balanced lower face ratio');

  // ── Facial thirds ─────────────────────────────────────────────────
  if (thirdsRatio < 0.75) weaknesses.push('Unbalanced facial thirds — lower face dominates');
  else if (thirdsRatio > 1.4)
    weaknesses.push('High forehead — upper third disproportionately large');

  // ── Eye spacing ───────────────────────────────────────────────────
  if (eyeSpacingRatio >= 0.29 && eyeSpacingRatio <= 0.36) strengths.push('Ideal eye spacing');
  else if (eyeSpacingRatio < 0.27)
    weaknesses.push('Close-set eyes — below-ideal interocular distance');
  else if (eyeSpacingRatio > 0.38)
    weaknesses.push('Wide-set eyes — above-ideal interocular distance');

  // ── Palpebral fissure (eye size) ──────────────────────────────────
  if (palpebralRatio >= 0.19 && palpebralRatio <= 0.24)
    strengths.push('Well-proportioned eye aperture');
  else if (palpebralRatio < 0.16)
    weaknesses.push('Small palpebral fissure — eyes appear smaller than ideal');

  // ── Nose width ────────────────────────────────────────────────────
  if (noseWidthRatio > 0.3)
    weaknesses.push('Wide nasal base — alar width exceeds ideal proportions');
  else if (noseWidthRatio < 0.2)
    weaknesses.push('Narrow nasal base — nose appears pinched relative to face');

  // ── Mouth / lips ──────────────────────────────────────────────────
  if (mouthWidthRatio >= 0.38 && mouthWidthRatio <= 0.46)
    strengths.push('Ideal mouth width proportions');
  else if (mouthWidthRatio < 0.34)
    weaknesses.push('Narrow mouth width — lips appear compressed relative to face');
  else if (mouthWidthRatio > 0.5)
    weaknesses.push('Wide mouth — horizontal lip span exceeds facial ideal');

  // ── Chin projection ───────────────────────────────────────────────
  if (chinProjection > 0.06)
    weaknesses.push('Recessed chin — reduced forward projection reduces lower-face definition');

  // ── Symmetry details ──────────────────────────────────────────────
  const jawSymmetryDetail = detailedSymmetry.find((s) => s.feature === 'Jawline');
  if (jawSymmetryDetail && jawSymmetryDetail.score < 75)
    weaknesses.push('Jaw asymmetry — lateral jaw landmark deviation detected');

  const eyeSymmetryDetail = detailedSymmetry.find((s) => s.feature === 'Eyes');
  if (eyeSymmetryDetail && eyeSymmetryDetail.score < 75)
    weaknesses.push('Eye-level asymmetry — orbital height imbalance detected');

  const cheekSymmetryDetail = detailedSymmetry.find((s) => s.feature === 'Cheekbones');
  if (cheekSymmetryDetail && cheekSymmetryDetail.score < 75)
    weaknesses.push('Cheekbone asymmetry — lateral midface imbalance detected');
