...
const BLOCKED_BOTS = [
  ...
  'Bytespider', 'Diffbot', 'FacebookBot', 'Omgilibot', 'Amazonbot',
  // Aggressive SEO crawlers
  'AhrefsBot', 'SemrushBot', 'MJ12bot', 'DotBot', 'BLEXBot',
  'DataForSeoBot', 'PetalBot', 'Seekport',
  // Scraping tools
...
const HEADLESS_INDICATORS = [
  ...
  'WebDriver', 'puppeteer', 'playwright', 'CasperJS',
  'Electron/', 'Slimer',
];
const HEADLESS_REGEX = new RegExp(HEADLESS_INDICATORS.map(b => b.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'), 'i');
...
export function blockBots(req: Request, res: Response, next: NextFunction) {
  ...
  if (isApiRoute) {
    ...
    if (score >= SUSPICION_THRESHOLD) {
      console.warn(`[Bot] Blocked suspicious request: score=${score} path=${req.path} ua="${ua.slice(0, 80)}"`);
      return res.status(403).json({ error: 'Forbidden' });
    }
  }
