...
import { trackActivity, getUserRiskStatus, generateDeviceFingerprint } from '../services/fraud.service.js';
 ...
 */
export function fraudCheck(action: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const clientFp = (req.headers['x-device-fingerprint'] as string) || req.body?._deviceFingerprint;
    ...
    try {
      // Check existing risk status (fast path)
      ...
      if (riskStatus.requiresCaptcha) {
        ...
        const { success, error } = await verifyTurnstileToken(captchaToken, ip, { trustedUserId: userId });
        if (!success) {
          return res.status(403).json({
...
export function requireTurnstile() {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const trustedUserId = (req as any).user?.uid;
    const { success, error } = await verifyTurnstileToken(token, ip || undefined, { trustedUserId });
    if (!success) {
      return res.status(403).json({
        error: error || 'CAPTCHA verification failed',
        code: 'CAPTCHA_FAILED',
